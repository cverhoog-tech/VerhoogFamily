...
module.exports = async function handler(req, res) {
  try {
    ...
    // FeedSharedData owns social-post persistence. FeedInteractionController owns
    // transient comment/GIF UI state so realtime snapshots cannot erase drafts.
    // HouseholdActivity stores immutable domain events; FeedActivityPresentation
    // consumes those events and merges them with social posts only at render time.
    html = html.replace(
      ...
      + '  <script src="src/modules/feed/feedInteractionController.js?v=1"></script>\n'
      + '  <script src="src/platform/activity/householdActivity.js?v=1"></script>\n'
      + '  <script src="src/modules/feed/feedActivityPresentation.js?v=1"></script>'
    );
