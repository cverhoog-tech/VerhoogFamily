...
(function(){
  ...
  function nameOf(id,fallback){var m=member(id);return (m&&(m.displayName||m.name))||fallback||tr('cleaning.familyMember','Gezinslid');}
  ...
  function selfName(){return nameOf(uid(),(window.fbUser&&window.fbUser.displayName)||window.myName||tr('cleaning.familyMember','Gezinslid'));}
  ...
  function chooseTarget(taskId){
    ...
    if(t.done){toast(tr('tasks.swap.completedNoSwap','Een voltooide taak kan niet worden geruild'));return;}
    if(!isAssignedTo(t,me)){toast(tr('tasks.swap.onlyOwn','Je kunt alleen een taak ruilen die aan jou is toegewezen'));return;}
    var existing=requestForTask(currentTaskId);if(existing){showRequest(existing);return;}
    var list=availableTargets(t);if(!list.length){toast(tr('tasks.swap.noTarget','Er is geen ander gezinslid beschikbaar voor deze ruil'));return;}
    var e=modal('<div class="tsr-head"><div class="tsr-crest">⇄</div><div><h3>'+esc(tr('tasks.swap.title','Taak ruilen'))+'</h3><p>'+esc(tr('tasks.swap.askTakeOver','Vraag een ander partylid om deze taak van je over te nemen.'))+'</p></div></div><div class="tsr-focus"><small>Quest</small><b>'+esc(t.title||tr('tasks.taskFallback','Taak'))+'</b></div><div class="tsr-list">'+list.map(function(m){var id=String(m.uid||m.id);return '<button class="tsr-person" data-target="'+esc(id)+'">'+avatarHtml(id)+'<span><b>'+esc(nameOf(id))+'</b><small>'+esc(tr('tasks.swap.sendRequest','Stuur ruilverzoek'))+'</small></span><span class="tsr-arrow">›</span></button>';}).join('')+'</div><div class="tsr-actions"><button class="tsr-btn tsr-muted" data-close>'+esc(tr('common.cancel','Annuleren'))+'</button></div>');
  ...
  function openPicker(){
    var me=uid();if(!me){toast(tr('tasks.swap.login','Log in om taken te ruilen'));return;}
    var list=(window.taskData||[]).filter(function(t){return !t.done&&isAssignedTo(t,me);});
    if(!list.length){toast(tr('tasks.swap.noOpen','Je hebt geen open taken om te ruilen'));return;}
    var e=modal('<div class="tsr-head"><div class="tsr-crest">⇄</div><div><h3>'+esc(tr('tasks.swap.title','Taak ruilen'))+'</h3><p>'+esc(tr('tasks.swap.chooseQuest','Kies welke quest je wilt overdragen aan een ander partylid.'))+'</p></div></div><div class="tsr-list">'+list.map(function(t){var id=String(t.id||t._key);return '<button class="tsr-person" data-task="'+esc(id)+'"><span class="tsr-avatar">⚔</span><span><b>'+esc(t.title||tr('tasks.taskFallback','Taak'))+'</b><small>'+esc(t.date||tr('tasks.noDate','Geen datum'))+'</small></span><span class="tsr-arrow">›</span></button>';}).join('')+'</div><div class="tsr-actions"><button class="tsr-btn tsr-muted" data-close>'+esc(tr('common.cancel','Annuleren'))+'</button></div>');
  ...
  function sendRequest(t,targetUid){
    ...
    var row={id:id,taskId:String(t.id||t._key),taskTitle:String(t.title||tr('tasks.taskFallback','Taak')),requesterUid:me,requesterName:selfName(),targetUid:String(targetUid),targetName:nameOf(targetUid),status:'pending',createdAt:firebase.database.ServerValue.TIMESTAMP,updatedAt:firebase.database.ServerValue.TIMESTAMP};
    base.child(id).set(row).then(function(){close();toast(tr('tasks.swap.requestSentTo','Ruilverzoek verstuurd naar '+nameOf(targetUid),{name:nameOf(targetUid)}));}).catch(function(){toast(tr('tasks.swap.sendFailed','Ruilverzoek versturen mislukt'));});
  }
  ...
  function showRequest(r){
    ...
    var requesterName=r.requesterName||nameOf(r.requesterUid),targetName=r.targetName||nameOf(r.targetUid);var e=modal('<div class="tsr-head"><div class="tsr-crest">⇄</div><div><h3>'+esc(incoming?tr('tasks.swap.requestTitle','Ruilverzoek'):tr('tasks.swap.requestSentTitle','Ruilverzoek verstuurd'))+'</h3><p>'+(incoming?'<b>'+esc(requesterName)+'</b> '+esc(tr('tasks.swap.requesterAsks',requesterName+' vraagt of jij deze taak wilt overnemen.',{name:requesterName}).replace(requesterName,'').trim()):esc(tr('tasks.swap.waitingFor','Wachten op antwoord van '+targetName+'.',{name:targetName})))+'</p></div></div><div class="tsr-focus"><small>Quest</small><b>'+esc(r.taskTitle||(t&&t.title)||tr('tasks.taskFallback','Taak'))+'</b></div><div class="tsr-person" style="pointer-events:none">'+avatarHtml(other)+'<span><b>'+esc(nameOf(other,incoming?r.requesterName:r.targetName))+'</b><small>'+esc(incoming?tr('tasks.swap.wantsSwap','Wil deze taak met jou ruilen'):tr('tasks.swap.recipient','Ontvanger van het verzoek'))+'</small></span></div><div class="tsr-note">'+esc(tr('tasks.swap.assignmentNote','De taak wordt pas opnieuw toegewezen nadat het verzoek is geaccepteerd.'))+'</div><div class="tsr-actions">'+(incoming?'<button class="tsr-btn tsr-muted" data-decline>'+esc(tr('tasks.swap.decline','Weigeren'))+'</button><button class="tsr-btn tsr-purple" data-accept>'+esc(tr('notif.accept','Accepteren'))+'</button>':'<button class="tsr-btn tsr-danger" data-cancel>'+esc(tr('tasks.swap.retract','Intrekken'))+'</button><button class="tsr-btn tsr-muted" data-close>'+esc(tr('common.close','Sluiten'))+'</button>')+'</div>');
  ...
  function respond(r,status){var d=db(),family=hid();if(!d||!family||!r)return Promise.reject(new Error(tr('tasks.swap.unavailable','Ruilverzoek niet beschikbaar')));return d.ref('families/'+family+'/taskSwapRequests/'+r.id).update({status:status,updatedAt:firebase.database.ServerValue.TIMESTAMP,respondedAt:firebase.database.ServerValue.TIMESTAMP}).then(function(){close();toast(status==='declined'?tr('tasks.swap.declined','Ruilverzoek geweigerd'):tr('tasks.swap.retracted','Ruilverzoek ingetrokken'));});}

  function accept(r){
    var t=task(r.taskId),me=uid();if(!t||!me||String(r.targetUid)!==String(me))return Promise.reject(new Error(tr('tasks.swap.notFound','Ruilverzoek niet gevonden')));
    if(!window.TaskSharedData||typeof TaskSharedData.update!=='function'){toast(tr('tasks.swap.dataNotReady','Taakdata is nog niet klaar'));return Promise.reject(new Error(tr('tasks.swap.dataNotReady','Taakdata is nog niet klaar')));}
    ...
    return TaskSharedData.update(t.id||t._key,{assignedToUids:next,assignedToUid:me,who:names}).then(function(){return db().ref('families/'+hid()+'/taskSwapRequests/'+r.id).update({status:'accepted',updatedAt:firebase.database.ServerValue.TIMESTAMP,respondedAt:firebase.database.ServerValue.TIMESTAMP,acceptedAt:firebase.database.ServerValue.TIMESTAMP});}).then(function(){close();toast(tr('tasks.swap.takenOver','Taak overgenomen: '+(t.title||tr('tasks.taskFallback','Taak')),{title:t.title||tr('tasks.taskFallback','Taak')}));}).catch(function(error){toast(tr('tasks.swap.failed','Taak ruilen mislukt'));throw error;});
  }
  ...
  function acceptRequest(id){var r=incomingPendingById(id);if(!r)return Promise.reject(new Error(tr('tasks.swap.inactive','Ruilverzoek is niet meer actief')));return accept(r);}
  function declineRequest(id){var r=incomingPendingById(id);if(!r)return Promise.reject(new Error(tr('tasks.swap.inactive','Ruilverzoek is niet meer actief')));return respond(r,'declined');}
  ...
  function decorateTrigger(){
    ...
    var b=document.createElement('button');b.type='button';b.className='tsr-trigger'+(requestForTask(currentTaskId)?' is-pending':'');b.setAttribute('aria-label',tr('tasks.swap.title','Taak ruilen'));b.title=tr('tasks.swap.title','Taak ruilen');b.textContent='⇄';b.onclick=function(e){e.preventDefault();e.stopPropagation();chooseTarget(currentTaskId);};hero.appendChild(b);
  }

  function snapshot(rows){
    var me=uid();requests=[];pendingIncoming=[];
    Object.keys(rows||{}).forEach(function(k){var r=rows[k];if(!r)return;if(!r.id)r.id=k;if(String(r.requesterUid)!==String(me)&&String(r.targetUid)!==String(me))return;requests.push(r);if(String(r.targetUid)===String(me)&&r.status==='pending')pendingIncoming.push(r);var prev=statusById[r.id];statusById[r.id]=r.status;if(prev&&prev!==r.status){if(String(r.requesterUid)===String(me)&&r.status==='accepted')toast(tr('tasks.swap.acceptedBy',(r.targetName||nameOf(r.targetUid))+' heeft “'+(r.taskTitle||tr('tasks.swap.taskFallback','de taak'))+'” overgenomen',{name:r.targetName||nameOf(r.targetUid),task:r.taskTitle||tr('tasks.swap.taskFallback','de taak')}));else if(String(r.requesterUid)===String(me)&&r.status==='declined')toast(tr('tasks.swap.declinedBy',(r.targetName||nameOf(r.targetUid))+' heeft het ruilverzoek geweigerd',{name:r.targetName||nameOf(r.targetUid)}));}});
    decorateTrigger();
  }
  ...
  window.addEventListener('familyapp:language-changed',function(){decorateTrigger();var modalEl=document.getElementById(modalId);if(modalEl)close();});
