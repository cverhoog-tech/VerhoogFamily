...
(function(){
  ...
  function refresh(list,meta,generation){
    ...
    all.forEach(function(q){var ev=q&&q.lastEvent,key=String(q&&q.id||q&&q._key||'');if(!key||!ev||!ev.id)return;if(!seenEvents[key]){seenEvents[key]=ev.id;return;}if(seenEvents[key]===ev.id)return;seenEvents[key]=ev.id;if(String(ev.actorUid||'')!==String(currentUid()||'')&&relevant(q))toast(ev.message||tr('party.updated','Party Quest bijgewerkt'));});
  ...
  function decorate(){var b=document.getElementById('tch-party-quest');if(!b||!active.length)return;var h=b.querySelector('b'),s=b.querySelector('small');if(h)h.textContent=active.length===1?tr('party.activeSingle','1 actieve Party Quest'):tr('party.activeMany',active.length+' actieve Party Quests',{count:active.length});if(s)s.textContent=tr('party.tapManage','Tik voor overzicht en beheer');}

  function leave(q){
    var s=service();if(!q||!s||typeof s.leaveQuest!=='function'){toast(tr('party.serviceNotReady','Party Quest service is nog niet klaar'));return Promise.resolve(false);}
    return s.leaveQuest(q.id||q._key).then(function(){dropLocal(q.id||q._key);toast(tr('party.leftQuest','Je hebt “'+(q.questTitle||'Party Quest')+'” verlaten',{quest:q.questTitle||'Party Quest'}));return true;}).catch(function(error){toast(failMessage(error,tr('party.leaveFailed','Party Quest verlaten mislukt')));return false;});
  }
  function end(q){
    var s=service();if(!q||!s||typeof s.cancelQuest!=='function'){toast(tr('party.serviceNotReady','Party Quest service is nog niet klaar'));return Promise.resolve(false);}
    return s.cancelQuest(q.id||q._key).then(function(){dropLocal(q.id||q._key);toast(tr('party.ended','Party Quest beëindigd'));return true;}).catch(function(error){toast(failMessage(error,tr('party.endFailed','Party Quest beëindigen mislukt')));return false;});
  }
  function startNew(){
    removeActiveOverlay();
    if(window.PartyQuestInvites&&typeof PartyQuestInvites.startNew==='function'){PartyQuestInvites.startNew();return true;}
    toast(tr('party.startUnavailable','Nieuwe Party Quest starten is nog niet beschikbaar'));return false;
  }
  ...
  function decorateDetail(){if(!openTaskId)return;var body=document.querySelector('#tdp-overlay .tdp-body');if(!body)return;var old=body.querySelector('.tdp-party-strip');if(old)old.remove();var q=active.find(function(x){return String(x.questId)===String(openTaskId);});if(!q)return;var people=participantRows(q);if(people.length<2)return;ensureDetailCss();var anchor=body.querySelector('.tdp-person');if(!anchor)return;var strip=document.createElement('div');strip.className='tdp-party-strip';strip.innerHTML='<div class="tdp-party-copy"><div class="tdp-party-label">'+esc(tr('party.groupQuest','⚔ Groepsquest'))+'</div><div class="tdp-party-sub">'+esc(people.map(function(p){return p.name;}).join(' & '))+'</div></div><div class="tdp-party-avatars">'+avatarStack(q,'tdp-party-avatar')+'</div>';anchor.parentNode.insertBefore(strip,anchor.nextSibling);}
  ...
  window.addEventListener('familyapp:language-changed',function(){decorate();decorateDetail();var openEl=document.getElementById('party-quest-active-view');if(openEl){openEl.remove();if(active.length)open();}});
