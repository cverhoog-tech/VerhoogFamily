'use strict';
// ============================================================
// PREMIUM TASK TABS v0.319
// Replaces the legacy task-tabs at runtime instead of stacking patches.
// The premium tabbar is the first child of #screen-tasks.
// ============================================================

(function(){
  var TABS = [
    { id: 'overzicht', label: 'Overzicht', icon: '' },
    { id: 'terugkerend', label: 'Terugkerend', icon: '🔁' },
    { id: 'persoon', label: 'Familie', icon: '👨‍👩‍👧' },
    { id: 'groupquests', label: 'Group', icon: '⚔️' },
    { id: 'help', label: 'Samen', icon: '🤝' },
    { id: 'preview', label: 'Preview', icon: '✨' }
  ];

  function currentTab(){ return window.taskTab || 'overzicht'; }
  ...
  function injectStyles(){
    ...
    s.textContent = [
      ...
      '#screen-tasks{margin-top:0!important;padding-top:0!important;overflow-x:hidden!important;max-width:100vw!important}',
      '#screen-tasks .task-tabs{display:none!important;visibility:hidden!important;height:0!important;min-height:0!important;max-height:0!important;overflow:hidden!important;margin:0!important;padding:0!important;border:0!important;box-shadow:none!important;position:absolute!important;pointer-events:none!important}',
      '.premiumTaskTabsShell{position:relative!important;z-index:80!important;width:100%!important;max-width:100%!important;margin:0!important;overflow:hidden!important;background:linear-gradient(180deg,rgba(255,255,255,.98),rgba(250,252,250,.96))!important;border-bottom:1px solid rgba(17,24,39,.08)!important;box-shadow:0 10px 22px rgba(17,24,39,.045)!important;isolation:isolate!important}',
      ...
      '.premiumTaskTab.active.overzicht{background:linear-gradient(135deg,#315f2c,#4f8a3c)!important}.premiumTaskTab.active.groupquests,.premiumTaskTab.active.preview{background:linear-gradient(135deg,#315f2c,#6d28d9)!important}',
      '#task-content{position:relative!important;z-index:1!important;max-width:100%!important;overflow-x:hidden!important;padding-top:16px!important;box-sizing:border-box!important}',
      '.tasks-overview,.group-quests-view,.task-page,.task-screen{max-width:100%!important;overflow-x:hidden!important;box-sizing:border-box!important}',
      '@media(max-width:420px){.premiumTaskTabsTrack{padding:8px 10px 9px!important;gap:8px!important}.premiumTaskTab{height:40px!important;min-width:92px!important;padding:0 13px!important;font-size:12.5px!important}.premiumTaskTab .ico{font-size:15px!important}#task-content{padding-top:14px!important}}'
    ].join('');
    document.head.appendChild(s);
  }

  function removeLegacyTabs(){
    document.querySelectorAll('#screen-tasks > .task-tabs').forEach(function(el){
      el.remove();
    });
  }
  ...
  function ensureShell(){
    ...
    if(screen.firstElementChild !== shell){
      screen.insertBefore(shell, screen.firstElementChild || null);
    }
    return shell;
  }
