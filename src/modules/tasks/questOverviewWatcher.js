'use strict';
// ============================================================
// QUEST OVERVIEW WATCHER v0.277
// Robust DOM-level integration for the v023 quest overlay.
// Adds smoother active group quest carousel + reversible help action.
// ============================================================
...
(function(){
  ...
  function injectStyles(){
    ...
    s.textContent = [
      ...
      '.gqOvShell{position:relative;overflow:hidden}',
      '.gqOvScroll{display:flex;gap:12px;overflow-x:auto;overflow-y:hidden;padding:0 2px 8px;scroll-snap-type:x proximity;-webkit-overflow-scrolling:touch;touch-action:pan-x;overscroll-behavior-x:contain;scroll-behavior:smooth}',
      '.gqOvScroll::-webkit-scrollbar{display:none}',
      '.gqOvCard{position:relative;overflow:hidden;flex:0 0 84%;min-height:160px;border:0;border-radius:24px;background-size:cover;background-position:center;color:#fff;box-shadow:0 18px 32px rgba(17,24,39,.16);scroll-snap-align:start;text-align:left;touch-action:manipulation}',
      ...
      '.gqOvControls button{width:34px;height:28px;border-radius:999px;border:0;background:#fff;color:#315f2c;font-size:16px;font-weight:950;box-shadow:0 6px 14px rgba(17,24,39,.08)}',
      ...
      '@media(max-width:420px){.gqOvCard{flex-basis:88%}.fqHelpMini2{font-size:9.5px;padding:5px 8px}}'
    ].join('');
  ...
  function scrollGqCarousel(dir){
    ...
    if(!scroller) return;
    var amount = Math.round(scroller.clientWidth * 0.86) * dir;
  ...
  function buildGroupOverview(){
    ...
    if(quests.length > 1){
      ...
      controls.innerHTML = '<button type="button" onclick="scrollGqCarousel(-1)">‹</button><button type="button" onclick="scrollGqCarousel(1)">›</button>';
      section.appendChild(controls);
    }
    return section;
  }

  function removeGroupQuestForSource(sourceTaskId){
    if(typeof window.loadGroupQuests !== 'function' || typeof window.saveGroupQuests !== 'function') return;
    var quests = window.loadGroupQuests().filter(function(q){ return q.sourceTaskId !== sourceTaskId; });
    window.saveGroupQuests(quests);
  }

  function unpromoteCard(card){
    var id = card.getAttribute('data-id') || '';
    savePromoted(loadPromoted().filter(function(x){ return x !== id; }));
    removeGroupQuestForSource(id);
    card.classList.remove('group-promoted');
    var badge = card.querySelector('.fqGroupBadge2');
    if(badge) badge.remove();
    var btn = card.querySelector('.fqHelpMini2');
    if(btn) btn.textContent = '⚔️ Vraag hulp';
    if(typeof window.showToast === 'function') window.showToast('Hulpvraag ingetrokken');
  ...
  function addHelpButton(card, promoted){
    ...
    var id = card.getAttribute('data-id') || '';
    btn.textContent = promoted.indexOf(id) > -1 ? '⚔️ Hulp gevraagd · intrekken' : '⚔️ Vraag hulp';
  }
  ...
  function augmentCards(){
    ...
    document.querySelectorAll('#task-content .fqCard').forEach(function(card){
      ...
      if(promoted.indexOf(id) > -1){
        ...
        card.classList.remove('group-promoted');
        var oldBadge = card.querySelector('.fqGroupBadge2');
  ...
  function boot(){
    ...
    if(root && !window.__questOverviewWatcherObserver){
      window.__questOverviewWatcherObserver = new MutationObserver(function(){
        ...
        window.__questOverviewWatcherTimer = setTimeout(applyOverviewPatch, 70);
      });
  ...
  setTimeout(boot, 300);
  setTimeout(boot, 900);
  window.addEventListener('familyapp:group-quests-updated', function(){ setTimeout(applyOverviewPatch, 70); });
})();
