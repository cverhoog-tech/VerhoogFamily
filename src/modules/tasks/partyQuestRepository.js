'use strict';
// ============================================================
// PARTY QUEST HOUSEHOLD REPOSITORY v1.0.0
// STEP 11.1 canonical Party Quest persistence/lifecycle boundary.
//
// Source of truth: families/{householdId}/partyQuests/{partyQuestId}
// Identity authority: HouseholdContext (UID + household + revision)
// Existing Party Quest v1 rows are normalized in memory only; STEP 11.1
// performs no eager migration and does not alter existing UI mutation flows.
// ============================================================
(function(){
  ...
  function safeKey(value){
    return String(value===undefined||value===null?'party_'+now():value).replace(/[.#$\[\]\/]/g,'_');
  }
  function db(){
    try{
  ...
  function rows(value){
    ...
    return Object.keys(value).map(function(key){
      var row=value[key];
      return row&&typeof row==='object'?{key:key,value:row}:null;
    }).filter(Boolean);
  }
  ...
  function normalizeExisting(row,key,ctx){
    ...
    if(out.id===undefined||out.id===null||out.id==='')out.id=canonicalKey;
  ...
  function sealMutation(server,changed,key,ctx){
    ...
    if(base.createdByUid)next.createdByUid=base.createdByUid;
  ...
  function subscribe(fn){
    ...
    subscribers.push(fn);
    try{fn(currentQuests.map(clone),clone(lastMeta));}catch(e){}
  ...
  function unbind(reason,clearProjection){
    ...
    active=null;
    bindGeneration++;
    if(clearProjection!==false)emit([],{source:reason||'unbound',ready:false,uid:null,householdId:null,revision:0});
  }
  function publish(binding,value,source){
  ...
  function bind(ctx,reason){
    ...
    if(!validContext(ctx)){
      emit([],{source:'context-not-ready',ready:false,uid:ctx&&ctx.uid||null,householdId:ctx&&ctx.householdId||null,revision:ctx&&ctx.revision||0});
      return false;
    }
    var database=db();
    if(!database){
      emit([],{source:'firebase-unavailable',ready:false,uid:ctx.uid,householdId:ctx.householdId,revision:ctx.revision,error:'FIREBASE_DATABASE_UNAVAILABLE'});
      return false;
    }
    var token=capture();
    ...
    var binding={
      ...
      ref:database.ref('families/'+ctx.householdId+'/partyQuests'),
      handler:null
    };
    active=binding;
    ...
    binding.handler=function(snapshot){
      if(!bindingCurrent(binding))return;
      publish(binding,snapshot&&snapshot.val?snapshot.val():null,'firebase');
    };
    binding.ref.on('value',binding.handler,function(error){
      if(!bindingCurrent(binding))return;
      emit([],{source:'firebase-error',ready:true,uid:binding.context.uid,householdId:binding.context.householdId,revision:binding.context.revision,error:error&&error.message||String(error||'PARTY_QUEST_LISTENER_ERROR')});
    });
    return true;
  }
  function handleContext(ctx,reason){
    if(!validContext(ctx)){
      if(active||currentQuests.length)unbind('context-cleared',true);
      return;
    }
  ...
  function requireBinding(){
    ...
    if(!active||!bindingCurrent(active))throw new Error('ACTIVE_PARTY_QUEST_HOUSEHOLD_REQUIRED');
    return active;
  }
  ...
  function assertWritable(binding){
    if(!bindingCurrent(binding))throw new Error('STALE_PARTY_QUEST_CONTEXT');
    if(rejectOffline())throw new Error('PARTY_QUEST_REPOSITORY_OFFLINE');
    return binding;
  }
  function findLocal(id){
    var wanted=String(id);
    return currentQuests.find(function(q){return String(q&&q.id)===wanted||String(q&&q._key)===wanted;})||null;
  }
  function keyFor(id){
    if(id&&typeof id==='object')return String(id._key||id.id||'');
    var local=findLocal(id);
    return local&&local._key?String(local._key):safeKey(id);
  }
  function mutateOne(id,mutator){
    var binding=assertWritable(requireBinding());
    var key=keyFor(id);
    if(!key)return Promise.reject(new Error('PARTY_QUEST_ID_REQUIRED'));
    ...
    return new Promise(function(resolve,reject){
      ...
      ref.transaction(function(server){
        ...
        var base=normalizeExisting(server,key,binding.context);
        var changed;
        ...
        if(!committed){reject(new Error(bindingCurrent(binding)?'PARTY_QUEST_NOT_FOUND_OR_ABORTED':'STALE_PARTY_QUEST_CONTEXT'));return;}
        if(!bindingCurrent(binding)){reject(new Error('STALE_PARTY_QUEST_CONTEXT'));return;}
  ...
  function status(){
    ...
    return {
      ...
      count:currentQuests.length,
      source:lastMeta.source||'idle',
      activePartyQuestListener:!!(active&&active.handler),
      canonicalPath:ctx&&ctx.householdId?'families/'+ctx.householdId+'/partyQuests':null
    };
  }

  window.PartyQuestRepository={
    version:VERSION,
    start:start,
    stop:stop,
    list:list,
