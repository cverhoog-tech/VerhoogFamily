...
function signInWithGoogle() {
  var authGate = window.AuthStartupReadinessGate;
  // Readiness + login-lock gate. Runs INSIDE signInWithGoogle itself (not
  // only via the disabled button) so a fast tap or a programmatic call
  // before the gate is ready can never start a half-initialized auth flow.
  if(authGate && !authGate.requestLaunch()) return;

  if(!fbAuth) {
    showAuthError('Firebase niet verbonden.');
    if(authGate) authGate.releaseLock();
    return;
  }
  ...
  if(isMobile) {
    var redirectPromise;
    try {
      redirectPromise = fbAuth.signInWithRedirect(provider);
    } catch(e) {
      showAuthError(translateFbError(e) + (e && e.code ? ' [' + e.code + ']' : ''));
      if(authGate) authGate.releaseLock();
      return;
    }
    if(redirectPromise && typeof redirectPromise.catch === 'function') {
      redirectPromise.catch(function(e) {
        // The redirect call itself failed before any navigation happened —
        // no navigation is coming, so the lock must be released again.
        showAuthError(translateFbError(e) + (e && e.code ? ' [' + e.code + ']' : ''));
        if(authGate) authGate.releaseLock();
      });
    }
    // On success, navigation is imminent — the lock intentionally stays
    // held until then. AuthSessionBootstrap owns everything after that.
  } else {
    fbAuth.signInWithPopup(provider)
      .then(function(result) {
        fbUser = result.user;
        if(authGate) authGate.markSuccess();
        // Household load + reveal owned by AuthSessionBootstrap via onAuthStateChanged.
      })
      .catch(function(e) {
