...
(function(){
  ...
  function render(el){
    ...
    if(!real.length&&!hydrated()){
      el.innerHTML='<div class="tch-page"'+(window.__taskVisualPolishV11?'':' style="visibility:hidden"')+'><div class="tch-page-title"><div><span>QUEST LOG</span><h2>Taken</h2></div><button class="tch-add-top" data-quick-add="1">＋</button></div>'+header()+filterBar()+loadingBody()+'<button class="tch-party-card" id="tch-party-quest"><span class="tch-party-icon">⚔️</span><span><b>Party Quests</b><small>Werk samen en verdien bonus XP</small></span><i>›</i></button></div>';
      bind(el);return;
    }
