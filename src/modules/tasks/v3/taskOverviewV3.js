...
(function(){
  ...
  function groupLabel(name){var map={Verlopen:'tasks.overdueTitle',Vandaag:'common.today',Morgen:'common.tomorrow',Later:'common.later',Voltooid:'tasks.section.completed'};return tr(map[name]||'',name);}
  ...
  function ensureStyle(){
    if(document.getElementById('task-overview-v32-style'))return;
    var style=document.createElement('style');style.id='task-overview-v32-style';
    style.textContent=''
      +'.tv3-filters{grid-template-columns:1.08fr repeat(5,minmax(0,1fr))!important}'
      +'.tv3-filters button{padding-left:3px!important;padding-right:3px!important;font-size:7.9px!important}'
      +'@media(max-width:390px){.tv3-filters{grid-template-columns:repeat(3,minmax(0,1fr))!important;row-gap:2px}.tv3-filters button{font-size:8.2px!important}}';
  ...
  function brandFamily(){var ms=M().members().slice(0,3),stack=ms.map(function(member){return avatarHtml({name:member.displayName||member.name||tr('cleaning.familyMember','Gezinslid'),avatar:M().avatar(member)},'tv3-family-avatar');}).join('');return '<button class="tv3-family-stack" data-tv3-members="1" aria-label="'+esc(tr('tasks.filterAria','Taken filter'))+'">'+(stack||ICON.people)+'</button>';}
  function brandHeader(){return '<header class="tv3-brand"><div class="tv3-brand-lockup"><span class="tv3-brand-leaf">'+ICON.leaf+'</span><span><strong>FamilyApp</strong><small>'+esc(tr('tasks.brand','Samen rust en overzicht'))+'</small></span></div><div class="tv3-brand-actions"><button class="tv3-icon-btn" data-tv3-notif="1" aria-label="'+esc(tr('nav.notifications','Meldingen'))+'">'+ICON.bell+'</button>'+brandFamily()+'</div></header>';}
  function memberMenu(){if(!state.memberMenu)return'';var rows=['<button data-tv3-person="all" class="'+(state.personUid==='all'?'active':'')+'">'+esc(tr('cleaning.everyone','Iedereen'))+'</button>'];M().members().forEach(function(member){var id=member.uid||member.id,name=member.displayName||member.name||tr('cleaning.familyMember','Gezinslid'),person={name:name,avatar:M().avatar(member)};rows.push('<button data-tv3-person="'+esc(id)+'" class="'+(String(state.personUid)===String(id)?'active':'')+'>'+avatarHtml(person,'tv3-menu-avatar')+'<span>'+esc(name)+'</span></button>');});rows.push('<button class="tv3-member-dashboard" data-tv3-person-dashboard="1">'+ICON.people+'<span>'+esc(tr('screen.profile','Profiel'))+'</span></button>');return '<div class="tv3-member-menu" role="menu">'+rows.join('')+'</div>';}
  function statusCards(summary){function card(type,value,label,icon){return '<button class="tv3-stat tv3-stat--'+type+'" data-tv3-stat="'+type+'"><span class="tv3-stat-icon">'+icon+'</span><strong>'+value+'</strong><small>'+label+'</small></button>';}return '<section class="tv3-stats">'+card('open',summary.open,tr('tasks.open','open taken'),ICON.clipboard)+card('important',summary.important,tr('tasks.important','belangrijk'),ICON.star)+card('overdue',summary.overdue,tr('tasks.overdue','verlopen'),ICON.clock)+'</section>';}
  function quote(){return '<section class="tv3-quote"><span class="tv3-quote-icon">'+ICON.leaf+'</span><span><strong>“'+esc(tr('tasks.quote.title','Kleine taken, een rustiger thuis'))+'”</strong><small>'+esc(tr('tasks.quote.subtitle','Samen maken we tijd voor wat echt telt.'))+'</small></span></section>';}
  function filters(){var items=[['all',tr('tasks.all','Alle taken')],['Vandaag',tr('common.today','Vandaag')],['Morgen',tr('common.tomorrow','Morgen')],['Later',tr('common.later','Later')],['cleaning',tr('nav.cleaning','Schoonmaken')],['Voltooid',tr('tasks.section.completed','Voltooid')]];return '<nav class="tv3-filters" aria-label="'+esc(tr('tasks.filterAria','Taken filter'))+'">'+items.map(function(x){return '<button data-tv3-filter="'+x[0]+'" class="'+(state.group===x[0]?'active':'')+'">'+x[1]+'</button>';}).join('')+'</nav>';}
  ...
  function isCleaning(vm){var t=vm&&vm.raw||{},source=String(t.sourceType||'').toLowerCase(),category=String(t.category||'').toLowerCase();return category==='cleaning'||source.indexOf('cleaning-occurrence')===0||!!(t.cleaningOccurrenceId||t.cleaningPlanId||t.cleaningRoomId||t.cleaningRoutineId);}
  function visibleVms(){return M().tasks().map(M().view).filter(assigneeMatches).filter(function(vm){if(state.group==='cleaning')return isCleaning(vm);if(state.group==='Vandaag')return vm.group==='Vandaag';return !isCleaning(vm);});}
  ...
  function shownGroups(all){if(state.group==='all'||state.group==='cleaning')return GROUP_ORDER.filter(function(name){return all[name].length;});return all[state.group]&&all[state.group].length?[state.group]:[];}
  function row(vm){var p=vm.primaryPerson,done=!!vm.raw.done;return '<article class="tv3-task-row'+(done?' is-done':'')+'" data-tv3-task="'+esc(vm.id)+'">'+'<button class="tv3-task-check" data-tv3-toggle="'+esc(vm.id)+'" aria-label="'+esc(done?tr('tasks.reopen','Heropen taak'):tr('tasks.complete','Voltooi taak'))+'" aria-pressed="'+(done?'true':'false')+'">'+(done?ICON.check:'')+'</button>'+'<div class="tv3-task-photo" style="background-image:url(\''+esc(vm.photo)+'\')" aria-hidden="true"></div>'+'<div class="tv3-task-copy"><strong>'+esc(vm.title)+'</strong><div class="tv3-task-meta">'+(p?avatarHtml(p,'tv3-row-avatar'):'')+(p?'<span>'+esc(p.name)+'</span><i></i>':'')+'<span class="tv3-date">'+ICON.calendar+esc(vm.dateLabel)+'</span></div></div>'+'<span class="tv3-xp">+'+vm.xp+' XP</span><span class="tv3-row-chevron">'+ICON.chevron+'</span></article>';}
  function groupSection(name,list){var open=state.expanded[name]!==false;return '<section class="tv3-group tv3-group--'+name.toLowerCase()+'"><button class="tv3-group-head" data-tv3-group="'+name+'"><span><strong>'+esc(groupLabel(name))+'</strong><small>'+esc(tr('tasks.count',list.length+' taken',{count:list.length}))+'</small></span><span class="tv3-group-arrow'+(open?' open':'')+'">'+ICON.down+'</span></button>'+(open?'<div class="tv3-list">'+list.map(row).join('')+'</div>':'')+'</section>';}
  function loading(){return '<div class="tv3-loading"><span>'+esc(tr('tasks.syncing','Taken synchroniseren…'))+'</span><i></i><i></i><i></i></div>';}
  function empty(){var message=state.group==='cleaning'?tr('tasks.noCleaning','Geen schoonmaaktaken gepland.'):tr('tasks.categoryCalm','Deze categorie is helemaal rustig.');return '<div class="tv3-empty"><strong>'+esc(tr('tasks.noneHere','Geen taken hier'))+'</strong><span>'+esc(message)+'</span></div>';}
  function top(summary){return brandHeader()+memberMenu()+'<section class="tv3-titlebar"><h1>'+esc(tr('nav.tasks','Taken'))+'</h1><button class="tv3-new" data-tv3-create="1">'+ICON.plus+'<span>'+esc(tr('tasks.new','Nieuwe taak'))+'</span></button></section>'+statusCards(summary)+quote()+filters();}

  function render(el){if(!el)el=document.getElementById('task-content');if(!el||!M())return;ensureStyle();document.body.setAttribute('data-task-v3','1');var summary=M().statusSummary();if(!M().tasks().length&&!M().isHydrated()){el.innerHTML='<div class="tv3-page">'+top(summary)+loading()+'</div>';bind(el);return;}var all=groups(),names=shownGroups(all),body=names.length?names.map(function(name){return groupSection(name,all[name]);}).join(''):empty();el.innerHTML='<div class="tv3-page">'+top(summary)+body+'</div>';bind(el);}
  ...
  var api={version:'3.2.0',render:render,state:state,isHydrated:function(){return M()&&M().isHydrated();}};
  ...
  ['familyapp:tasks-updated','familyapp:household-identity-synced','familyapp:party-quests-updated','familyapp:cleaning-plan-reconciled','familyapp:cleaning-derived-cleanup','familyapp:language-changed'].forEach(function(name){window.addEventListener(name,rerender);});
})();