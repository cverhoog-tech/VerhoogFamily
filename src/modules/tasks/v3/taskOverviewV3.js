...
(function(){
  ...
  function filters(){var items=[['all','Alle taken'],['Vandaag','Vandaag'],['Morgen','Morgen'],['Later','Later'],['cleaning','Schoonmaken'],['Voltooid','Voltooid']];return '<nav class="tv3-filters" aria-label="Taken filter">'+items.map(function(x){return '<button data-tv3-filter="'+x[0]+'" class="'+(state.group===x[0]?'active':'')+'">'+x[1]+'</button>';}).join('')+'</nav>';}
  ...
  function empty(){var message=state.group==='cleaning'?'Geen schoonmaaktaken gepland.':'Deze categorie is helemaal rustig.';return '<div class="tv3-empty"><strong>Geen taken hier</strong><span>'+message+'</span></div>';}
  ...
  ['familyapp:tasks-updated','familyapp:household-identity-synced','familyapp:party-quests-updated','familyapp:cleaning-plan-reconciled','familyapp:cleaning-derived-cleanup'].forEach(function(name){window.addEventListener(name,rerender);});
})();