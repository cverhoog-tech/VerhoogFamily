'use strict';
// ============================================================
// GROUP QUESTS PREMIUM UX v0.272
// Visual override for the first Group Quest foundation.
// Keeps existing mechanics, upgrades presentation.
// ============================================================
...
(function(){
  var BG = [
    ...
    'https://images.unsplash.com/photo-1455390582262-044cdead277a?auto=format&fit=crop&w=1200&q=92&fm=webp'
  ];

  function pickBg(q, i){
    var t = ((q.title || '') + ' ' + (q.description || '')).toLowerCase();
    if(t.indexOf('boodschap') > -1 || t.indexOf('grocery') > -1) return BG[1];
    if(t.indexOf('reset') > -1 || t.indexOf('schoon') > -1 || t.indexOf('clean') > -1) return BG[0];
    if(t.indexOf('planning') > -1 || t.indexOf('agenda') > -1) return BG[3];
    return BG[i % BG.length];
  }

  function injectPremiumStyles(){
    if(document.getElementById('group-quests-premium-styles')) return;
    ...
    s.textContent = [
      ...
      '.gq-hero:before{content:"";position:absolute;inset:0;background:radial-gradient(circle at top right,rgba(255,255,255,.30),transparent 34%),linear-gradient(180deg,transparent,rgba(0,0,0,.22));pointer-events:none}',
      '.gq-hero>*{position:relative;z-index:1}.gq-hero p{margin:0 0 7px;font-size:11px;font-weight:950;letter-spacing:.14em;opacity:.76}.gq-hero h2{margin:0;font-size:29px;letter-spacing:-.7px;line-height:1}.gq-hero span{display:block;margin-top:8px;font-size:13px;line-height:1.35;opacity:.86;max-width:230px}',
      '.gq-hero button{border:0;border-radius:999px;background:rgba(255,255,255,.94);color:#173e17;padding:12px 15px;font-size:13px;font-weight:950;white-space:nowrap;box-shadow:0 10px 22px rgba(0,0,0,.22);cursor:pointer;backdrop-filter:blur(12px)}',
      ...
      '.gq-card.premium{position:relative;overflow:hidden;border:0;border-radius:28px;margin-bottom:14px;min-height:300px;background-size:cover;background-position:center;box-shadow:0 24px 44px rgba(17,24,39,.18),0 2px 0 rgba(255,255,255,.55) inset;isolation:isolate;transform:translateZ(0)}',
      '.gq-card.premium.completed{opacity:.78}.gq-card-shade{position:absolute;inset:0;background:linear-gradient(180deg,rgba(0,0,0,.08),rgba(0,0,0,.48) 44%,rgba(12,18,16,.84)),radial-gradient(circle at 82% 10%,rgba(255,255,255,.26),transparent 28%);z-index:0}.gq-card-content{position:relative;z-index:1;min-height:300px;display:flex;flex-direction:column;justify-content:flex-end;padding:18px;color:#fff}',
      '.gq-card-top{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:9px}.gq-card h3{margin:5px 0 0;font-size:23px;color:#fff;letter-spacing:-.55px;line-height:1.05;text-shadow:0 2px 10px rgba(0,0,0,.24)}.gq-card p{margin:0 0 13px;font-size:13px;line-height:1.42;color:rgba(255,255,255,.82);max-width:94%}',
      '.gq-card strong{background:linear-gradient(135deg,#fef3c7,#facc15);color:#2d2100;border-radius:999px;padding:8px 11px;font-size:12px;white-space:nowrap;box-shadow:0 9px 18px rgba(250,204,21,.24)}.gq-type{display:inline-flex;align-items:center;gap:4px;font-size:10px;text-transform:uppercase;letter-spacing:.11em;font-weight:950;color:rgba(255,255,255,.78)}',
      '.gq-party-row{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px}.gq-party-row>span{font-size:11px;font-weight:900;color:rgba(255,255,255,.78);background:rgba(255,255,255,.14);border:1px solid rgba(255,255,255,.18);border-radius:999px;padding:6px 9px;backdrop-filter:blur(12px)}',
      ...
      '.gq-steps{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px}.gq-steps span{font-size:10px;font-weight:850;color:rgba(255,255,255,.70);background:rgba(0,0,0,.18);border:1px solid rgba(255,255,255,.10);border-radius:999px;padding:5px 8px}.gq-steps span.done{color:#dcfce7;background:rgba(22,163,74,.24);border-color:rgba(134,239,172,.28)}',
      '.gq-party{display:flex;min-height:40px}.gq-avatar{width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#3f7f2f,#6d28d9);background-size:cover;background-position:center;border:2px solid rgba(255,255,255,.94);margin-right:-8px;color:#fff;display:flex;align-items:center;justify-content:center;position:relative;box-shadow:0 9px 18px rgba(0,0,0,.25)}.gq-avatar.empty{background:rgba(255,255,255,.14);border-style:dashed}.gq-avatar span{font-size:11px;font-weight:950}.gq-avatar.has-img span{display:none}.gq-avatar small{position:absolute;right:-3px;bottom:-3px;min-width:17px;height:17px;border-radius:50%;background:#fff;color:#24521f;border:1px solid rgba(220,232,215,.92);font-size:9px;font-weight:950;display:flex;align-items:center;justify-content:center}',
      ...
      '.task-tabs .ttab.gq-tab{background:linear-gradient(135deg,#315f2c,#6d28d9);color:#fff;border-color:transparent;box-shadow:0 8px 18px rgba(49,95,44,.18)}@media(max-width:420px){.gq-hero{display:block;min-height:155px}.gq-hero button{margin-top:14px;width:100%}.gq-card.premium,.gq-card-content{min-height:316px}.gq-actions{flex-direction:column}.gq-card h3{font-size:22px}}'
    ].join('');
    document.head.appendChild(s);
  }

  function premiumMembers(members){
    if(typeof getGroupQuestMember !== 'function') return '';
    if(!members || !members.length) return '<div class="gq-avatar empty"><span>+</span><small>0</small></div>';
    return members.slice().sort(function(a,b){return (b.contribution||0)-(a.contribution||0);}).map(function(entry){
  ...
  function renderPremiumGroupQuests(container){
    ...
    var html = '<div class="group-quests-view premium">'
      + '<section class="gq-hero"><div><p>FAMILY RAID BOARD</p><h2>Group quests</h2><span>Teamwork quests, party progress en gezamenlijke XP.</span></div><button onclick="createDemoGroupQuest()">+ Nieuwe raid</button></section>'
      + '<div class="gq-summary-grid"><div><b>'+open.length+'</b><span>open raids</span></div><div><b>'+joins+'</b><span>party joins</span></div><div><b>'+totalXp+'</b><span>XP pool</span></div></div>';

    if(!open.length) html += '<div class="gq-empty">Geen open group quests. Maak een nieuwe family raid aan.</div>';
    ...
    open.forEach(function(q, i){
      ...
      var bg = q.background || pickBg(q, i);
      html += '<article class="gq-card premium" style="background-image:url('+bg+')"><div class="gq-card-shade"></div><div class="gq-card-content">'
        + '<div class="gq-card-top"><div><span class="gq-type">⚔️ '+(q.difficulty || (q.xp >= 260 ? 'Raid' : 'Party'))+' · '+(q.type || 'weekly')+'</span><h3>'+q.title+'</h3></div><strong>'+reward.total+' XP</strong></div>'
        + '<p>'+q.description+'</p>'
        + '<div class="gq-party-row"><div class="gq-party">'+premiumMembers(q.members)+'</div><span>'+getGroupQuestPartySize(q)+' joined</span></div>'
        ...
        + premiumSteps(q)
        + '<div class="gq-actions">'+(joined ? '<button class="ghost" onclick="leaveGroupQuest(\''+q.id+'\')">Leave party</button>' : '<button class="ghost" onclick="joinGroupQuest(\''+q.id+'\')">Join party</button>')+'<button onclick="contributeGroupQuest(\''+q.id+'\')">+ Bijdrage</button></div>'
        + '</div></article>';
    });

    if(completed.length){
      html += '<h3 class="gq-section-title">Voltooid</h3>';
      completed.forEach(function(q, i){
        var reward = getGroupQuestReward(q);
        html += '<article class="gq-card premium completed" style="background-image:url('+pickBg(q, i)+')"><div class="gq-card-shade"></div><div class="gq-card-content"><div class="gq-card-top"><div><span class="gq-type">🏆 completed</span><h3>'+q.title+'</h3></div><strong>'+reward.total+' XP</strong></div><p>Team quest afgerond. Klaar voor activity log, rewards en achievements.</p></div></article>';
      });
    }
    html += '</div>';
