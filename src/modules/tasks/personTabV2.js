...
(function(){
  ...
  function initials(member){
    if(member&&member.initials)return member.initials;
    return String(member&&member.displayName||'?').split(/\s+/).filter(Boolean)
      .map(function(part){return part.charAt(0);}).join('').slice(0,2).toUpperCase()||'?';
  }
  function currentModel(models){
    if(!models.length)return null;
    var chosen=selectedUid&&models.find(function(model){return model.uid===selectedUid;});
    if(!chosen)chosen=models.find(function(model){return model.member&&model.member.isCurrent;})||models[0];
    selectedUid=chosen.uid;
    return chosen;
  }
  function presenceState(model){return String(model&&model.presence&&model.presence.state||'offline');}
  function presenceText(model){
    var p=model&&model.presence||{};
    if(p.state==='online')return p.area?'Online · '+String(p.area):'Online';
    if(p.state==='recent')return'Recent actief';
    if(p.state==='today')return'Vandaag actief';
    return'Offline';
  }
  function avatarHtml(model,large){
    var member=model.member||{};
    var src=member.avatar||'';
    var cls=large?'pt2-avatar pt2-avatar-large':'pt2-avatar';
    if(src)return'<img class="'+cls+'" src="'+esc(src)+'" alt="'+esc(member.displayName||'Gezinslid')+'">';
    return'<div class="'+cls+' pt2-avatar-fallback">'+esc(initials(member))+'</div>';
  }
  function memberRail(models){
    return'<div class="pt2-members" role="list">'+models.map(function(model){
      var member=model.member||{},active=model.uid===selectedUid,state=presenceState(model);
      return'<button type="button" class="pt2-member'+(active?' is-active':'')+'" data-pt2-member="'+esc(model.uid)+'" role="listitem">'
        +'<span class="pt2-avatar-shell">'+avatarHtml(model,false)+'<i class="pt2-status-dot is-'+esc(state)+'"></i></span>'
        +'<span class="pt2-member-name">'+esc(member.displayName||'Gezinslid')+'</span>'
        +'<span class="pt2-member-state">'+esc(state==='online'?'Online':state==='recent'?'Recent':state==='today'?'Vandaag':'Offline')+'</span>'
        +'</button>';
    }).join('')+'</div>';
  }
  function hero(model){
    ...
    return'<section class="pt2-hero">'
      ...
      +'<div class="pt2-hero-body">'
        +'<div class="pt2-hero-identity">'+avatarHtml(model,true)+'<div><h2>'+esc(member.displayName||'Gezinslid')+'</h2><p>'+esc(progress.title||'Avonturier')+'</p></div></div>'
