...
(function(){
  ...
  function addMealPlanToShop(){var visibleDates={};weekDays(weekOffset).forEach(function(d){visibleDates[iso(d)]=true;});var planned=rows().filter(function(m){return m&&m.recipeId&&visibleDates[m.date];});if(!planned.length){if(typeof window.showToast==='function')window.showToast('Geen recepten gepland in deze week');return;}var svc=window.ShoppingListService,lists=svc&&svc.list?svc.list():[];if(!svc||!lists.length){if(typeof window.showToast==='function')window.showToast('Open eerst een boodschappenlijst');return;}var target=(window.ShoppingLists&&ShoppingLists.active?ShoppingLists.active():null)||lists[0],key=target&&target.key;if(!key)return;var chain=Promise.resolve(),added=0;planned.forEach(function(m){var rec=recipeById(m.recipeId);if(!rec)return;chain=chain.then(function(){return svc.appendRecipeIngredients(key,rec).then(function(result){added+=result&&result.added?result.added.length:0;});});});chain.then(function(){if(typeof window.showToast==='function')window.showToast(added+' ingrediënten toegevoegd');if(typeof window.showScreenMore==='function')window.showScreenMore('shop');});}
