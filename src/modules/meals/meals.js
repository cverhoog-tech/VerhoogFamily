...
(function(){
  ...
  function fmt(d){return d.toLocaleDateString(locale(),{day:'numeric',month:'short'}).replace('.','');}
  ...
  function openPlanner(date,type,recipeId){var p=window.MealPlannerBottomSheetBridge;if(p&&typeof p.openMealPlanner==='function'){p.openMealPlanner({date:date,mealType:type,recipeId:recipeId||null});return;}if(typeof window.showToast==='function')window.showToast(tr('meals.loading','Maaltijdplanner wordt geladen'));}

  function renderMeals(){
    var el=document.getElementById('meals-content');if(!el)return;ensureStyles();
    var days=weekDays(weekOffset),dayNames=days.map(function(d){try{return d.toLocaleDateString(locale(),{weekday:'short'}).replace('.','');}catch(error){return'';}});
    var html='<div class="meal-section-title">'+icon('utilityCalendar','md')+'<span>'+tr('meals.weekMenu','Week menu')+'</span></div>'
      +'<div class="meal-week-switch"><button type="button" data-week="0" class="'+(weekOffset===0?'active':'')+'">'+tr('meals.thisWeek','Deze week')+'<span>'+weekRange(0)+'</span></button><button type="button" data-week="1" class="'+(weekOffset===1?'active':'')+'">'+tr('meals.nextWeek','Volgende week')+'<span>'+weekRange(1)+'</span></button></div>'
      +'<div style="padding:0 16px 8px;font-size:12px;color:var(--c-text2)">'+tr('meals.hint','Tik op een slot om een recept of maaltijd te plannen.')+'</div>';
    days.forEach(function(d,i){
      var ds=iso(d),lunch=mealAt(ds,'lunch'),dinner=mealAt(ds,'dinner'),isToday=ds===today();
      function slot(type,m,labelKey,labelFallback){var rec=m&&m.recipeId?recipeById(m.recipeId):null,title=m?(m.title||(rec&&rec.name)||tr('meals.meal','Maaltijd')):null;return '<div class="meal-slot" data-date="'+ds+'" data-slot="'+type+'"><span class="meal-slot-icon">'+foodIcon(type,'sm')+'</span>'+(m?'<div class="meal-slot-name">'+title+'</div><button type="button" data-clear-date="'+ds+'" data-clear-slot="'+type+'" style="background:none;border:none;font-size:12px;color:var(--c-text3);cursor:pointer;margin-left:auto">✕</button>':'<div class="meal-slot-empty">'+tr(labelKey,labelFallback)+'</div>')+'</div>';}
      html+='<div class="meal-day" style="'+(isToday?'background:var(--c-primary-light)':'')+'"><div class="meal-day-label" style="'+(isToday?'color:var(--c-primary);font-weight:800':'')+'">'+dayNames[i]+'<br><span style="font-size:10px;font-weight:400">'+d.getDate()+'/'+(d.getMonth()+1)+'</span></div><div style="flex:1;display:flex;flex-direction:column;gap:5px">'+slot('lunch',lunch,'meals.lunchChoose','Lunch kiezen...')+slot('dinner',dinner,'meals.dinnerChoose','Diner kiezen...')+'</div></div>';
    });
    html+='<div style="padding:12px 16px"><button id="meal-week-shop" style="width:100%;background:var(--c-primary);color:#fff;border:none;border-radius:12px;padding:12px;font-size:14px;font-weight:700;cursor:pointer"><span class="meal-shop-icon">'+icon('utilityShopping','sm')+'</span>'+tr('meals.addWeekIngredients','Voeg ingrediënten van deze week toe')+'</button></div>';
    el.innerHTML=html;
...
window.addEventListener('familyapp:language-changed',function(){try{if(typeof renderMeals==='function')renderMeals();}catch(error){}});
