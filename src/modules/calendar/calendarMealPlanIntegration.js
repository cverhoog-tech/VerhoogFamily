'use strict';
// ============================================================
// CALENDAR MEAL PLAN INTEGRATION v1.0
// Projects the existing mealPlanData into Agenda without duplicating
// records into calData. Meal planner remains the source of truth.
// ============================================================
(function(){
  if(window.CalendarMealPlanIntegration)return;
  var VERSION='1.0.0',STYLE_ID='calendar-meal-plan-integration-style';
  var originalRenderCal=window.renderCal;
  var originalRenderCalEvents=window.renderCalEvents;

  function esc(v){return String(v==null?'':v).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}
  function meals(){return Array.isArray(window.mealPlanData)?window.mealPlanData:[];}
  ...
  function ensureStyles(){
    ...
    var s=document.createElement('style');s.id=STYLE_ID;s.textContent=''
      ...
      +'.cal-day.has-meal:not(.today):after{background:#d59a32!important}.cal-day.has-event.has-meal:not(.today):after{box-shadow:4px 0 0 #7c5ce0}'
  ...
  function markMealDays(){
    ...
    var first=new Date(window.calYear,window.calMonth,1),startDow=(first.getDay()||7)-1,daysInMonth=new Date(window.calYear,window.calMonth+1,0).getDate();
    ...
    for(var d=1;d<=daysInMonth;d++){
      ...
      var cell=cells[d-1];if(cell&&dayMeals(date).length)cell.classList.add('has-meal');
    }
  }
  ...
  function renderCal(){
    if(typeof originalRenderCal==='function')originalRenderCal();
    markMealDays();
  }
  ...
  function openRecipe(encodedId){
    ...
    if(typeof window.openRecipeDetail==='function'){
      if(typeof window.showScreen==='function')window.showScreen('recipes');
      setTimeout(function(){window.openRecipeDetail(id);},80);
      return;
    }
