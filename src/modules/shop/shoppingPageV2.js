...
(function(){
  ...
  function rowHtml(item){var key=String(item._key||item.id||''),safe=domKey(key);return '<div class="shopv2-item" data-item-key="'+esc(key)+'" id="shopv2-item-'+safe+'">'
    +'<button type="button" class="shopv2-item-main" data-action="toggle" data-item-key="'+esc(key)+'" aria-label="'+esc(item.done?tr('shop.toggleOpen','Terug naar te kopen: '+item.name,{name:item.name}):tr('shop.toggleBought','Markeer gekocht: '+item.name,{name:item.name}))+'">'
    ...
    +'<button type="button" class="shopv2-delete" data-action="delete" data-item-key="'+esc(key)+'" aria-label="'+esc(tr('shop.deleteItem','Verwijder '+item.name,{name:item.name}))+'">×</button></div>';}
  ...
  function openCreateList(){var st=store();if(!st||!window.BottomSheet)return;window.BottomSheet.open({title:tr('shop.newShoppingList','Nieuwe winkellijst'),html:'<div class="fam-modal-field"><label>'+tr('shop.name','Naam')+'</label><input id="shopv2-new-list-name" placeholder="'+tr('shop.placeholderWeekly','bijv. Weekboodschappen')+'"></div><div class="fam-modal-field"><label>'+tr('shop.visibility','Zichtbaarheid')+'</label><select id="shopv2-new-list-privacy"><option value="household">'+tr('shop.household','Gezin')+'</option><option value="private">'+tr('shop.onlyMe','Alleen ik')+'</option></select></div>',actions:[{label:tr('common.cancel','Annuleren')},{label:tr('shop.createList','Lijst maken'),primary:true,onClick:function(ctx){var name=ctx.modal.querySelector('#shopv2-new-list-name').value.trim();if(!name)return false;st.createList({name:name,visibility:ctx.modal.querySelector('#shopv2-new-list-privacy').value}).catch(function(){toast(tr('shop.listCreateFailed','Lijst kon niet worden aangemaakt'));});}}]});}
  function openAdd(){if(window.GroceryAddSheet&&typeof window.GroceryAddSheet.open==='function')window.GroceryAddSheet.open();else toast(tr('shop.addLoading','Toevoegen wordt geladen…'));}
