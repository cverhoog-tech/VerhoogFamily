...
(function(){
  ...
  function catLabel(value){var map={Groente:'grocery.cat.vegetables',Fruit:'grocery.cat.fruit',Zuivel:'grocery.cat.dairy',Brood:'grocery.cat.bread',Vlees:'grocery.cat.meat',Dranken:'grocery.cat.drinks',Overig:'grocery.cat.other'};return tr(map[value]||'',value);}
  function familyLabel(){return tr('auth.family','FAMILIE').toLowerCase().replace(/^./,function(x){return x.toUpperCase();});}
  ...
  function shopItemHTML(item){
    ...
    return '<div class="shop-item" id="si-' + domKey + '">'
      ...
      + '<div class="shop-info"><div class="shop-name' + (item.done ? ' done' : '') + '">' + esc(item.name) + '</div><div class="shop-qty">' + esc(item.qty) + ' · ' + esc(catLabel(displayCategory(item))) + '</div></div>'
  ...
  function renderListSwitcher(view){var bar=ensureListBar();if(!bar||!view.key){if(bar)bar.innerHTML='';return;}var displayName=view.name==='Gezinslijst'?tr('shop.familyList','Gezinslijst'):(view.name||tr('shop.shoppingList','Winkellijst'));bar.innerHTML='<button class="shopping-listpick" id="shopping-list-pick" aria-label="'+esc(tr('shop.chooseListAria','Winkellijst kiezen'))+'"><span class="sl-icon">'+listScopeIcon(view.scope)+'</span><span class="sl-copy"><b>'+esc(displayName)+'</b><small>'+(view.scope==='private'?esc(tr('shop.private','Privé')):esc(tr('shop.liveHousehold','Gezin · live')))+' · '+esc(tr('shop.toBuyCount',view.openCount+' te kopen',{count:view.openCount}))+'</small></span><span class="sl-chevron">⌄</span></button>';var pick=document.getElementById('shopping-list-pick');if(pick)pick.onclick=openPicker;}
  function openPicker(){var s=store();if(!s||!window.BottomSheet)return;var rowsHtml=s.all().map(function(row){var rowName=row.list.name==='Gezinslijst'?tr('shop.familyList','Gezinslijst'):row.list.name;return '<button class="shopping-list-option" data-list-key="'+esc(row.key)+'"><span class="sl-icon">'+uiIcon('utilityShopping','sm')+'</span><span style="flex:1"><b>'+esc(rowName)+'</b><small>'+(row.scope==='private'?esc(tr('shop.onlyMe','Alleen ik')):esc(tr('shop.liveHousehold','Gezin · live')))+'</small></span></button>';}).join('')+'<button class="shopping-list-create" id="shopping-list-create">'+esc(tr('shop.newList','＋ Nieuwe lijst'))+'</button>';window.BottomSheet.open({title:tr('shop.chooseList','Winkellijst kiezen'),html:rowsHtml,onOpen:function(ctx){ctx.modal.querySelectorAll('[data-list-key]').forEach(function(btn){btn.onclick=function(){s.setActiveList(btn.getAttribute('data-list-key'));ctx.close();};});var create=ctx.modal.querySelector('#shopping-list-create');if(create)create.onclick=function(){ctx.close();setTimeout(openCreate,180);};},actions:[{label:tr('common.close','Sluiten')}]});}
  function openCreate(){var s=store();if(!s||!window.BottomSheet)return;var html='<div class="fam-modal-field"><label>'+esc(tr('add.name','Naam'))+'</label><input id="sl-name" placeholder="'+esc(tr('shop.listNamePlaceholder','bijv. IKEA of Weekboodschappen'))+'"></div><div class="fam-modal-field"><label>'+esc(tr('shop.visibility','Zichtbaarheid'))+'</label><select id="sl-privacy"><option value="household">'+esc(tr('auth.family','FAMILIE').toLowerCase().replace(/^./,function(x){return x.toUpperCase();}))+'</option><option value="private">'+esc(tr('shop.onlyMe','Alleen ik'))+'</option></select></div>';window.BottomSheet.open({title:tr('shop.newListTitle','Nieuwe winkellijst'),html:html,actions:[{label:tr('common.cancel','Annuleren')},{label:tr('shop.createList','Lijst maken'),primary:true,onClick:function(ctx){var name=ctx.modal.querySelector('#sl-name').value.trim();if(!name)return false;s.createList({name:name,visibility:ctx.modal.querySelector('#sl-privacy').value}).catch(function(){if(typeof window.showToast==='function')window.showToast(tr('shop.listCreateFailed','Lijst kon niet worden aangemaakt'));});}}]});}
  ...
  function toggleShop(id){
    ...
    work.then(function(record){if(record&&record.done){if(typeof window.awardXP==='function')window.awardXP(2,tr('search.type.grocery','Boodschap'));if(typeof window.addActivity==='function')window.addActivity('🛒','#fff3dc',tr('shop.boughtActivity',(window.myName||familyLabel())+' kocht “'+record.name+'”',{name:window.myName||familyLabel(),item:record.name}));}}).catch(function(err){console.warn('[Shop] toggle failed',err);interactionPendingUntil=0;scheduleRenderShop(true);if(typeof window.showToast==='function')window.showToast(tr('shop.updateItemFailed','Kon item niet bijwerken. Probeer opnieuw.'));});return false;
  }
  function deleteShop(id){var s=store();if(!s)return false;s.deleteItem(id).catch(function(err){console.warn('[Shop] delete failed',err);if(typeof window.showToast==='function')window.showToast(tr('shop.deleteItemFailed','Kon item niet verwijderen. Probeer opnieuw.'));});return false;}
  function resetShop(){var s=store();if(!s)return false;s.clearDone().then(function(){if(typeof window.showToast==='function')window.showToast(tr('shop.cleared','Gekochte items geleegd ↺'));}).catch(function(err){console.warn('[Shop] reset failed',err);if(typeof window.showToast==='function')window.showToast(tr('shop.clearDoneFailed','Kon niet legen. Probeer opnieuw.'));});return false;}
  ...
  function niceNumber(value){var n=Math.round(Number(value)*100)/100;try{return n.toLocaleString(locale(),{maximumFractionDigits:2});}catch(error){return String(n);}}
  ...
  function conflictHtml(analysis){return '<div class="shopping-conflict-intro"><b>'+esc(tr('shop.duplicateIntro',analysis.conflicts.length+' dubbele producten gevonden. Kies per product wat er moet gebeuren. Niets wordt stil overschreven.',{count:analysis.conflicts.length}))+'</b></div><div class="shopping-conflict-list">'+analysis.conflicts.map(function(c,i){var selected=c.canSum?'sum':'separate';return '<div class="shopping-conflict-row"><div class="shopping-conflict-copy"><div><b>'+esc(c.incoming.name)+'</b><small>'+esc(tr('shop.onList','Op lijst: '+qtyLabel(c.existing),{qty:qtyLabel(c.existing)}))+' · '+esc(tr('shop.fromRecipe','Recept: '+qtyLabel(c.incoming),{qty:qtyLabel(c.incoming)}))+'</small></div></div><select data-conflict-index="'+i+'">'+(c.canSum?'<option value="sum"'+(selected==='sum'?' selected':'')+'>'+esc(tr('shop.conflict.sum','Optellen → '+c.sumQty,{qty:c.sumQty}))+'</option>':'')+'<option value="replace">'+esc(tr('shop.conflict.replace','Bestaande hoeveelheid vervangen'))+'</option><option value="separate"'+(selected==='separate'?' selected':'')+'>'+esc(tr('shop.conflict.separate','Als aparte regel toevoegen'))+'</option><option value="skip">'+esc(tr('shop.conflict.skip','Bestaande laten staan'))+'</option></select></div>';}).join('')+'</div>';}
  function applyRecipeConflictPlan(s,analysis,decisions){
    var repo=repository();if(!repo||typeof repo.setItem!=='function')return Promise.reject(new Error(tr('shop.storageUnavailable','Boodschappenopslag niet beschikbaar')));var additions=analysis.unique.slice(),processed=[],skipped=[],chain=Promise.resolve();
  ...
  function openDuplicateResolver(s,analysis){
    ...
    ensureStyles();return new Promise(function(resolve,reject){var modal=window.BottomSheet.open({title:tr('shop.conflictTitle','Dubbele boodschappen'),html:conflictHtml(analysis),closeOnBackdrop:false,actions:[{label:tr('shop.onlyNew','Alleen nieuwe'),keepOpen:true,onClick:function(ctx){var decisions={};analysis.conflicts.forEach(function(_,i){decisions[i]='skip';});ctx.modal.querySelectorAll('.fam-modal-btn').forEach(function(b){b.disabled=true;});applyRecipeConflictPlan(s,analysis,decisions).then(function(result){ctx.close();resolve(result);}).catch(function(error){ctx.modal.querySelectorAll('.fam-modal-btn').forEach(function(b){b.disabled=false;});reject(error);});}},{label:tr('shop.applyChoices','Keuzes toepassen'),primary:true,keepOpen:true,onClick:function(ctx){var decisions={};ctx.modal.querySelectorAll('[data-conflict-index]').forEach(function(sel){decisions[parseInt(sel.getAttribute('data-conflict-index'),10)]=sel.value;});ctx.modal.querySelectorAll('.fam-modal-btn').forEach(function(b){b.disabled=true;});applyRecipeConflictPlan(s,analysis,decisions).then(function(result){ctx.close();resolve(result);}).catch(function(error){ctx.modal.querySelectorAll('.fam-modal-btn').forEach(function(b){b.disabled=false;});reject(error);});}}]});if(!modal)reject(new Error(tr('shop.conflictWindowFailed','Conflictvenster kon niet openen')));});
  }
  ...
  function wireShopAddButton(){var screen=document.getElementById('screen-shop');if(!screen)return;var header=screen.querySelector('.list-header');if(!header)return;var btn=header.querySelector('.add-btn');if(!btn){btn=document.createElement('button');btn.className='add-btn';header.appendChild(btn);}btn.textContent='+ '+tr('common.add','Toevoegen');btn.onclick=function(e){if(e)e.preventDefault();if(window.GroceryAddSheet&&typeof window.GroceryAddSheet.open==='function')window.GroceryAddSheet.open();return false;};}
  ...
  window.addEventListener('familyapp:language-changed',function(){try{renderShop();wireShopAddButton();}catch(error){}});
