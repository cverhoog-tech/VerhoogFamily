...
(function(){
  ...
  function conflictHtml(analysis){return '<div class="shopping-conflict-intro"><b>'+esc(tr('shop.duplicateIntro',analysis.conflicts.length+' dubbele producten gevonden. Kies per product wat er moet gebeuren. Niets wordt stil overschreven.',{count:analysis.conflicts.length}))+'</b></div><div class="shopping-conflict-list">'+analysis.conflicts.map(function(conflict,index){var selected=conflict.canSum?'sum':'separate';return '<div class="shopping-conflict-row"><div class="shopping-conflict-copy"><div><b>'+esc(conflict.incoming.name)+'</b><small>'+esc(tr('shop.onList','Op lijst: '+qtyLabel(conflict.existing),{qty:qtyLabel(conflict.existing)}))+' · '+esc(tr('shop.fromRecipe','Recept: '+qtyLabel(conflict.incoming),{qty:qtyLabel(conflict.incoming)}))+'</small></div></div><select data-conflict-index="'+index+'">'+(conflict.canSum?'<option value="sum"'+(selected==='sum'?' selected':'')+'>'+esc(tr('shop.conflict.sum','Optellen → '+conflict.sumQty,{qty:conflict.sumQty}))+'</option>':'')+'<option value="replace">'+esc(tr('shop.conflict.replace','Bestaande hoeveelheid vervangen'))+'</option><option value="separate"'+(selected==='separate'?' selected':'')+'>'+esc(tr('shop.conflict.separate','Als aparte regel toevoegen'))+'</option><option value="skip">'+esc(tr('shop.conflict.skip','Bestaande laten staan'))+'</option></select></div>';}).join('')+'</div>';}

  function applyPlan(s,analysis,decisions){
    var repo=repository();if(!repo||typeof repo.setItem!=='function')return Promise.reject(new Error(tr('shop.storageUnavailable','Boodschappenopslag niet beschikbaar')));
  ...
  function openResolver(s,analysis){
    ...
    ensureStyles();return new Promise(function(resolve,reject){var modal=window.BottomSheet.open({title:tr('shop.conflictTitle','Dubbele boodschappen'),html:conflictHtml(analysis),closeOnBackdrop:false,onOpen:function(ctx){if(ctx&&ctx.modal)ctx.modal.classList.add('shopping-conflict-modal');},actions:[{label:tr('shop.onlyNew','Alleen nieuwe'),keepOpen:true,onClick:function(ctx){var decisions={};analysis.conflicts.forEach(function(_,index){decisions[index]='skip';});ctx.modal.querySelectorAll('.fam-modal-btn').forEach(function(button){button.disabled=true;});applyPlan(s,analysis,decisions).then(function(result){ctx.close();resolve(result);}).catch(function(error){ctx.modal.querySelectorAll('.fam-modal-btn').forEach(function(button){button.disabled=false;});reject(error);});}},{label:tr('shop.applyChoices','Keuzes toepassen'),primary:true,keepOpen:true,onClick:function(ctx){var decisions={};ctx.modal.querySelectorAll('[data-conflict-index]').forEach(function(select){decisions[parseInt(select.getAttribute('data-conflict-index'),10)]=select.value;});ctx.modal.querySelectorAll('.fam-modal-btn').forEach(function(button){button.disabled=true;});applyPlan(s,analysis,decisions).then(function(result){ctx.close();resolve(result);}).catch(function(error){ctx.modal.querySelectorAll('.fam-modal-btn').forEach(function(button){button.disabled=false;});reject(error);});}}]});if(!modal)reject(new Error(tr('shop.conflictWindowFailed','Conflictvenster kon niet openen')));});
  }
