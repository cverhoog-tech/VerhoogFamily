...
(function(){
  ...
  function isExpanded(id){return !!state.expandedComments[postId(id)];}
  function setExpanded(id,value){state.expandedComments[postId(id)]=!!value;}
  function draft(id){return state.commentDrafts[postId(id)]||'';}
  function gifDraft(id){return state.commentGifDrafts[postId(id)]||'';}
  function setDraft(id,value){state.commentDrafts[postId(id)]=String(value==null?'':value);}
  function setGifDraft(id,value){var key=postId(id);if(value)state.commentGifDrafts[key]=String(value);else delete state.commentGifDrafts[key];}
  function rerender(){try{if(typeof window.renderFeed==='function')window.renderFeed();}catch(e){console.warn('[FeedInteractionController] render failed',e);}}
