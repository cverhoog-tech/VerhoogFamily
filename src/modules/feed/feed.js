...
// Feed data is now owned by FeedSharedData (src/modules/feed/feedSharedData.js),
// which is the sole writer of families/{householdId}/shared/feedPosts and keeps
// window.feedData in sync as a realtime, household-shared projection. feed.js
...
function currentUid(){
  try { return (window.fbUser||(window.firebase&&firebase.auth&&firebase.auth().currentUser)||{}).uid||null; }
  catch(e) { return null; }
}
function canDeleteFeedPost(p){
  try { if(window.FeedSharedData&&typeof window.FeedSharedData.canDelete==='function')return window.FeedSharedData.canDelete(p); }
  catch(e) {}
  return true;
}
...
function commentsHTML(p){var cs=p.comments||[];if(!p._showComments||!cs.length)return '';return '<div class="fs-comments">'+cs.map(function(c){
  var gifHtml = c.gifUrl ? '<img src="'+c.gifUrl+'" style="max-width:200px;border-radius:12px;margin-top:6px;display:block;">' : '';
  return '<div class="fs-comment">'+avatarHTML(c.author,c.initials,c.color,'fs-comment-avatar')+'<div><b>'+escHtml(c.author)+'</b>'+(c.text?'<span>'+escHtml(c.text)+'</span>':'')+gifHtml+'<small>'+escHtml(c.time||'nu')+' &nbsp; ♡ '+((c.likes||[]).length||0)+'</small></div></div>';
}).join('')+'</div>';}
...
function toggleLike(id){
  ...
  window.FeedSharedData.toggleReaction(id).then(function(r){
    ...
    // so the tap feels instant even before the Firebase echo arrives.
    renderFeed();
  }).catch(function(err){
...
function submitComment(id){
  ...
  // Check ook voor gif
  var gifPreview = document.getElementById('gif-preview-'+id);
  var gifUrl = gifPreview && gifPreview.dataset.gifUrl;
  ...
  window.FeedSharedData.addComment(id,{text:text,gifUrl:gifUrl||null}).then(function(){
    inp.disabled = false;
    inp.value = '';
    if(gifPreview){ gifPreview.style.display='none'; gifPreview.dataset.gifUrl=''; gifPreview.innerHTML=''; }
    p._showComments = true;
    renderFeed();
    if(typeof addNotif==='function'&&!isOwnFeedAuthor(p.author)) addNotif('💬','#f3e8ff',profileName()+' reageerde op je post', text||'[GIF]');
  }).catch(function(err){
    inp.disabled = false;
...
function publishPost(){
  ...
  var linkedEntity=composeLinkedTask?clone(composeLinkedTask):null;
  window.FeedSharedData.createPost({text:text,media:media,mediaType:composeMediaType,linkedEntity:linkedEntity}).then(function(created){
    // Only clear the composer and show success feedback once the post is
    // actually confirmed written — a fake success here is exactly the bug
    // this rebuild exists to fix.
    if(ca)ca.textContent='';
    clearComposeMedia();
    clearFeedStatus();
    var justPosted=created&&feedData.find(function(p){return String(p.id)===String(created.id);});
    if(justPosted)justPosted._showComments=true;
    renderFeed();
    if(typeof updateStats==='function')updateStats();
...
/* ── GIF PICKER ── */
var _gifPickerId = null;
function openGifPicker(postId) {
  _gifPickerId = String(postId);
  var existing = document.getElementById('gif-picker-modal');
  if(existing) existing.remove();
