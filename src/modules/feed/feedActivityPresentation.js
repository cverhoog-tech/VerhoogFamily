'use strict';
// ============================================================
// FEED ACTIVITY PRESENTATION v1.1.0
...
(function(){
  ...
  var PRESENTATIONS={
    ...
    'meal.planned':function(e){var p=e.payload||{};return{eyebrow:'DINNER PLANNED',title:(p.mealName||'Maaltijd')+' staat op het menu',body:'Gepland door '+actorName(e),detail:p.plannedDate||null,meta:p.slot==='lunch'?'Lunch':(p.slot==='breakfast'?'Ontbijt':(p.slot==='dinner'?'Diner':null)),icon:'🍽️',tone:'meal-planned'};},
  ...
  function card(event){var presenter=PRESENTATIONS[event.type];if(!presenter)return '';var vm=presenter(event);return '<article class="fs-card fs-activity '+esc(vm.tone)+'" data-activity-id="'+esc(event.id)+'" onclick="openHouseholdActivitySource(\''+esc(event.id)+'\')">'
  ...
  function feedBody(){var st=null;try{st=window.FeedSharedData&&FeedSharedData.status?FeedSharedData.status():null;}catch(e){}if(!st||!st.hasSnapshot)return '<div class="fs-feed-loading" style="text-align:center;padding:48px 20px;color:var(--c-text2,#6b7280)"><div style="font-size:28px;margin-bottom:8px">⏳</div><div style="font-size:14px;font-weight:600">Feed wordt verbonden...</div></div>';var items=merged();if(window.feedFilter==='tasks')items=items.filter(function(x){return (x.kind==='activity'&&(x.value.type==='task.created'||x.value.type==='task.completed'))||(x.kind==='post'&&x.value.type==='task');});if(window.feedFilter==='agenda')items=items.filter(function(x){return x.kind==='post'&&x.value.type==='agenda';});if(!items.length)return '<div class="fs-feed-empty" style="text-align:center;padding:48px 20px;color:var(--c-text2,#6b7280)"><div style="font-size:28px;margin-bottom:8px">💬</div><div style="font-size:14px;font-weight:600">Nog geen berichten</div><div style="font-size:12px;margin-top:4px">Deel iets met het gezin hierboven</div></div>';return items.map(function(item){if(item.kind==='activity')return card(item.value);return typeof window.renderPostHTML==='function'?window.renderPostHTML(item.value):'';}).join('');}

  function openSource(eventId){var event=events().find(function(e){return String(e.id)===String(eventId);});if(!event||!event.source)return;var module=event.source.module;try{if(module==='tasks'){if(typeof window.showScreen==='function')window.showScreen('tasks');return;}if(module==='meals'){if(typeof window.showScreenMore==='function')window.showScreenMore('meals');else if(typeof window.showScreen==='function')window.showScreen('meals');return;}if(module==='shop'){if(typeof window.showScreenMore==='function')window.showScreenMore('shop');else if(typeof window.showScreen==='function')window.showScreen('shop');}}catch(e){}}

  function css(){if(document.getElementById('feed-activity-presentation-v2'))return;var s=document.createElement('style');s.id='feed-activity-presentation-v2';s.textContent=''
    +'.fs-activity{position:relative;overflow:hidden;cursor:pointer}.fs-activity:active{transform:scale(.992)}.fs-activity:before{content:"";position:absolute;inset:0 auto 0 0;width:4px;background:#7c3aed}.fs-activity.task-created:before{background:#6d5bd0}.fs-activity.task-completed:before{background:#c69024}.fs-activity.meal-planned:before{background:#d66b4b}.fs-activity.shopping-completed:before{background:#3f7f2f}'
  ...
  window.FeedActivityPresentation={version:'1.1.0',presentations:PRESENTATIONS,merged:merged,renderCard:card,openSource:openSource};
})();
