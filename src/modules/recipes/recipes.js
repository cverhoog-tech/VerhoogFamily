...
// Volledig herschreven. Geen bridges, geen patches.
// ============================================================
(function () {
  ...
  // ── DATA ─────────────────────────────────────────────────
  var R = []; // recipes array
  var checkedI = {}; // {id: Set<index>} ingredients
  var checkedS = {}; // {id: Set<index>} steps
  ...
  function saveData() {
    ...
    if (window.HouseholdRepository && window.HouseholdRepository.write)
      try { window.HouseholdRepository.write('recipes', R, {source:'recipes'}); } catch(e) {}
  }
  ...
  function addCSS() {
    ...
    s.textContent = ''
      ...
      // Chips container uses existing .chips class
      ;
  ...
  function renderList() {
    ...
    // Build the entire list view HTML fresh
    screen.innerHTML = ''
      + '<div id="recipe-list-view">'
      // Header
      + '<div class="list-header">'
      + '<h2>Recepten</h2>'
      + '<button id="r-addbtn" onclick="window._rAddNew()">+ Recept</button>'
      + '</div>'
      // Search
      + '<div id="rs-wrap"><input id="rs-inp" placeholder="🔎 Zoek recept, keuken of ingrediënt..." autocomplete="off"></div>'
      // Chips
      + '<div class="chips" style="padding:4px 14px 10px">'
      + '<button class="chip'+(currentFilter==='all'?' active':'')+'" onclick="window._rFilter(\'all\')">Alle</button>'
      + '<button class="chip'+(currentFilter==='Diner'?' active':'')+'" onclick="window._rFilter(\'Diner\')">🍽️ Diner</button>'
      + '<button class="chip'+(currentFilter==='Ontbijt'?' active':'')+'" onclick="window._rFilter(\'Ontbijt\')">🥞 Ontbijt</button>'
      + '<button class="chip'+(currentFilter==='Lunch'?' active':'')+'" onclick="window._rFilter(\'Lunch\')">🥗 Lunch</button>'
      + '<button class="chip'+(currentFilter==='Snack'?' active':'')+'" onclick="window._rFilter(\'Snack\')">🍿 Snack</button>'
      + '<button class="chip'+(currentFilter==='Dessert'?' active':'')+'" onclick="window._rFilter(\'Dessert\')">🍰 Dessert</button>'
      + '<button class="chip'+(currentFilter==='Bakken'?' active':'')+'" onclick="window._rFilter(\'Bakken\')">🧁 Bakken</button>'
      + '</div>'
      // Grid
      + '<div id="rg"></div>'
      + '</div>'
    ...
    // Wire search
    var inp = document.getElementById('rs-inp');
    if (inp) {
  ...
  function renderGrid() {
    ...
    if (!list.length) {
      grid.innerHTML = '<div class="r-empty">Geen recepten gevonden</div>';
      return;
    }
    ...
    grid.innerHTML = list.map(function(r) {
      ...
      return '<div class="rc" onclick="window._rOpen(\''+esc(r.id)+'\')">'
        + (p
            ? '<div class="rc-img" style="background-image:url(\''+esc(p)+'\')"></div><div class="rc-ov"></div>'
            : '<div class="rc-emoji">'+e+'</div>')
        + '<div class="rc-top">'
        + '<span class="rc-tag">'+esc(r.cuisine||r.cat)+'</span>'
        + '<span class="rc-tag">⏱ '+esc(r.time||20)+'m</span>'
        + '</div>'
        + '<div class="rc-bot">'
        + '<div class="rc-name">'+esc(r.name)+'</div>'
        + '<div class="rc-sub">'
        + '<span>'+e+' '+esc(r.cat)+'</span>'
        + '<span>👥 '+esc(r.persons||4)+'p</span>'
        + '</div></div></div>';
    }).join('');
  }
  ...
  function renderDetail(id) {
    ...
    var ings = (r.ingredients||[]).map(function(ing, i) {
      var on = ci.has(i);
      return '<div class="rd-ing" data-i="'+i+'">'
        +'<div class="rd-dot'+(on?' on':'')+'">'+( on?SVG:'')+'</div>'
        +'<div class="rd-ing-txt'+(on?' on':'')+'">'+esc(ing)+'</div>'
        +'</div>';
    }).join('') || '<p style="color:var(--c-text2);font-size:13px">Geen ingrediënten</p>';

    var steps = (r.steps||[]).map(function(step, i) {
      var on = cs.has(i);
      return '<div class="rd-step" data-s="'+i+'">'
        +'<div class="rd-dot'+(on?' on':'')+'">'+( on?SVG:'')+'</div>'
        +'<div class="rd-stepn">'+(i+1)+'</div>'
        +'<div class="rd-step-txt'+(on?' on':'')+'">'+esc(step)+'</div>'
        +'</div>';
    ...
    dv.innerHTML = ''
      // Back
      + '<div class="list-header" style="padding:10px 14px 6px">'
      + '<button class="add-btn" style="background:var(--c-surface2);color:var(--c-text);box-shadow:none" onclick="window._rBack()">← Terug</button>'
      + '</div>'
      // Hero
      + '<div id="rd-hero">'
      + (p ? '<img src="'+esc(p)+'" onerror="this.style.display=\'none\'">' : e)
      + '<button class="rd-photobtn" onclick="window._rPhoto(\''+esc(id)+'\')">📷 Foto</button>'
      + '</div>'
      // Info
      + '<div class="rd-info">'
      + '<h2>'+esc(r.name)+'</h2>'
      ...
      + (r.cuisine ? '<span class="recipe-tag">🌍 '+esc(r.cuisine)+'</span>' : '')
      + '</div></div>'
      // Actions
      + '<div class="rd-actions">'
      + '<button class="rd-btn pri" onclick="window._rEdit(\''+esc(id)+'\')">✏️ Bewerken</button>'
      + '<button class="rd-btn red" onclick="window._rDel(\''+esc(id)+'\')">🗑️ Verwijderen</button>'
      + '</div>'
      // Ingredients
      + '<div class="rd-sec">'
      + '<div class="rd-sec-h">Ingrediënten</div>'
      + ings
      + '<button class="rd-shopbtn" onclick="window._rShop(\''+esc(id)+'\')">🛒 Alles op boodschappenlijst</button>'
      + '</div>'
      // Steps
      + '<div class="rd-sec">'
      + '<div class="rd-sec-h">Bereiding</div>'
      + steps
      + '</div>'
    ...
    dv.querySelectorAll('[data-i]').forEach(function(row) {
    ...
    // Step checkboxes
    dv.querySelectorAll('[data-s]').forEach(function(row) {
      row.onclick = function() {
        var i = +row.getAttribute('data-s');
  ...
  // ── WINDOW HANDLERS (hardcoded onclick safe) ─────────────
  window._rFilter = function(cat) {
    currentFilter = cat;
    renderList();
  };

  window._rOpen = function(id) {
    renderDetail(id);
  };

  window._rBack = function() {
    var dv = document.getElementById('recipe-detail-view');
    var lv = document.getElementById('recipe-list-view');
  ...
  window._rAddNew = function() {
    openEditor(null);
  };

  window._rEdit = function(id) {
    openEditor(id);
  };

  window._rDel = function(id) {
    var r = R.find(function(x){ return String(x.id)===String(id); });
  ...
  window._rPhoto = function(id) {
    var r = R.find(function(x){ return String(x.id)===String(id); });
    if (!r || !window.BottomSheet) return;
    window.BottomSheet.open({
      title: '📷 Foto wijzigen',
      html: '<div class="fam-modal-field"><label>Foto URL</label><input id="rp-url" value="'+esc(r.photo||'')+'"></div>'
        +'<div style="margin-bottom:10px"><button type="button" class="add-btn" id="rp-upbtn" style="margin-right:8px">📂 Upload</button>'
        +'<input type="file" accept="image/*" id="rp-file" style="display:none"></div>',
      onOpen: function(ctx) {
        var ub = ctx.modal.querySelector('#rp-upbtn');
        var fi = ctx.modal.querySelector('#rp-file');
        if (ub&&fi) { ub.onclick=function(){fi.click();}; fi.onchange=function(e){var f=e.target.files[0];if(!f)return;var rd=new FileReader();rd.onload=function(ev){var u=ctx.modal.querySelector('#rp-url');if(u)u.value=ev.target.result;};rd.readAsDataURL(f);}; }
      },
      actions:[
        {label:'Annuleren'},
        {label:'Opslaan',primary:true,onClick:function(ctx){
          ...
          setTimeout(function(){renderDetail(id);},80);
          return true;
        }}
      ]
    });
  ...
  function openEditor(id) {
    ...
    window.BottomSheet.open({
      ...
      html: '<div class="fam-modal-field"><label>Naam</label><input id="re-name" value="'+esc(r?r.name:'')+'"></div>'
        ...
        +'<div class="fam-modal-field"><label>Personen</label><input id="re-pers" type="number" value="'+esc(r?r.persons:4)+'"></div>'
        +'<div class="fam-modal-field"><label>Tijd (min)</label><input id="re-time" type="number" value="'+esc(r?r.time:30)+'"></div>'
        +'</div>'
        ...
        {label:'Opslaan',primary:true,onClick:function(ctx){
          ...
          if(r){
            ...
            r={id:newId(),name:name,cat:m.querySelector('#re-cat').value,
              ...
              emoji:CAT_ICONS[m.querySelector('#re-cat').value]||'🍴',photo:null,
              ingredients:ings,
  ...
  window.closeRecipeDetail = function(){ window._rBack(); };
  window.renderRecipeGrid  = renderGrid;
  window.recipesData       = R;

  // Boot: load data now so it's ready
  loadData();
  seedData();
