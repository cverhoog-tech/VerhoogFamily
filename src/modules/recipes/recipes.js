...
(function(){
  ...
  function addCSS(){
    if(document.getElementById('rcss2'))return;var s=document.createElement('style');s.id='rcss2';s.textContent=[
      ...
      '.recipe-import-action{padding:2px 14px 10px}',
      '#r-import-link-btn{display:flex;align-items:center;justify-content:center;gap:7px;width:100%;border:0;border-radius:15px;padding:12px 16px;background:linear-gradient(135deg,#3b82f6,#1d4ed8);color:#fff;font-size:13.5px;font-weight:800;cursor:pointer;box-shadow:0 6px 16px rgba(29,78,216,.28)}',
  ...
  function renderList(){
    ...
    screen.innerHTML='<div id="recipe-list-view"><div class="list-header"><h2>Recepten</h2><button id="r-addbtn">+ Recept</button></div><div id="rs-wrap"><input id="rs-inp" placeholder="Zoek recept, keuken..." autocomplete="off"></div><div class="recipe-import-action"><button type="button" id="r-import-link-btn">🔗 Importeren via link</button></div><div class="chips" style="padding:4px 14px 10px"><button class="chip" id="rf-all">Alle</button><button class="chip" id="rf-diner">Diner</button><button class="chip" id="rf-ontbijt">Ontbijt</button><button class="chip" id="rf-lunch">Lunch</button><button class="chip" id="rf-snack">Snack</button><button class="chip" id="rf-dessert">Dessert</button><button class="chip" id="rf-bakken">Bakken</button></div><div id="rg"></div></div><div id="recipe-detail-view" style="display:none"></div>';
  ...
  function openShoppingPicker(r){
    var s=window.ShoppingListStore;if(!s||!window.BottomSheet){if(typeof window.showToast==='function')window.showToast('Boodschappenlijsten zijn nog niet beschikbaar');return;}var rows=s.all();
    if(!rows.length){if(typeof window.showToast==='function')window.showToast('Maak eerst een boodschappenlijst via Boodschappen');if(typeof window.showScreenMore==='function')window.showScreenMore('shop');return;}
    ...
    window.BottomSheet.open({title:'🛒 Kies boodschappenlijst',html:html,onOpen:function(ctx){ctx.modal.querySelectorAll('[data-list]').forEach(function(btn){btn.onclick=function(){var key=btn.getAttribute('data-list');btn.disabled=true;s.appendRecipeIngredients(r,key).then(function(result){ctx.close();if(typeof window.showToast==='function')window.showToast(result.added.length+' toegevoegd'+(result.skipped.length?' · '+result.skipped.length+' bestond al':'')+' ✓');}).catch(function(){btn.disabled=false;if(typeof window.showToast==='function')window.showToast('Toevoegen mislukt');});};});},actions:[{label:'Annuleren'}]});
  }
