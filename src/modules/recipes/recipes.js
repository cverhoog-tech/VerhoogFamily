...
(function(){
  ...
  var HERO_PRESETS={
    Ontbijt:'assets/recipes/hero-breakfast.webp',
    Lunch:'assets/recipes/hero-lunch.webp',
    Diner:'assets/recipes/hero-dinner.webp',
    Snack:'assets/recipes/hero-lunch.webp',
    Dessert:'assets/recipes/hero-breakfast.webp',
    Bakken:'assets/recipes/hero-breakfast.webp'
  };
  ...
  function heroFor(r){if(r&&r.photo)return r.photo;return HERO_PRESETS[r&&r.cat]||HERO_PRESETS.Diner;}
  function heroFallback(r){var e=r&&r.emoji||CAT_ICONS[r&&r.cat]||'🍴';return '<div class="rc-emoji">'+e+'</div>';}
  ...
  function renderGrid(){
    ...
    grid.innerHTML=rows.map(function(r){var p=heroFor(r),e=r.emoji||CAT_ICONS[r.cat]||'🍴',id=esc(r.id);return '<div class="rc" id="rc-'+id+'"><div class="rc-img"><img src="'+esc(p)+'" alt="" loading="lazy" onerror="this.parentNode.outerHTML=\''+heroFallback(r).replace(/'/g,"\\'")+'\'"></div><div class="rc-ov"></div><div class="rc-top"><span class="rc-tag">'+esc(r.cuisine||r.cat)+'</span><span class="rc-tag">⏱ '+esc(r.time||20)+'m</span></div><div class="rc-bot"><div class="rc-name">'+esc(r.name)+'</div><div class="rc-sub"><span>'+e+' '+esc(r.cat)+'</span><span>👥 '+esc(r.persons||4)+'p</span></div></div></div>';}).join('');
    rows.forEach(function(r){var el=document.getElementById('rc-'+String(r.id));if(el)el.onclick=function(){renderDetail(r.id);};});
  }

  function renderDetail(id){
    syncFromStore();var r=getRecipe(id);if(!r)return;if(!checkedI[id])checkedI[id]=new Set();if(!checkedS[id])checkedS[id]=new Set();var ci=checkedI[id],cs=checkedS[id],p=heroFor(r),e=r.emoji||CAT_ICONS[r.cat]||'🍴';
  ...
  function openEditor(id){
    var r=id?getRecipe(id):null;if(!window.BottomSheet)return;var existingIngredients=r?ingredientLines(r.ingredients):[];
    window.BottomSheet.open({title:r?'✏️ Bewerken':'🍳 Nieuw recept',html:'<div class="fam-modal-field"><label>Naam</label><input id="re-name" value="'+esc(r?r.name:'')+'"></div><div class="fam-modal-field"><label>Maaltijdtype</label><select id="re-cat"><option>Ontbijt</option><option>Lunch</option><option>Diner</option><option>Snack</option><option>Dessert</option><option>Bakken</option></select></div><div class="fam-modal-field"><label>Keuken</label><input id="re-cuis" value="'+esc(r?r.cuisine||'':'')+'"></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px"><div class="fam-modal-field"><label>Personen</label><input id="re-pers" type="number" value="'+esc(r?r.persons:4)+'"></div><div class="fam-modal-field"><label>Tijd (min)</label><input id="re-time" type="number" value="'+esc(r?r.time:30)+'"></div></div><div class="fam-modal-field"><label>Ingrediënten · 1 per regel</label><textarea id="re-ings" rows="6">'+esc(existingIngredients.join('\n'))+'</textarea></div><div class="fam-modal-field"><label>Stappen · 1 per regel</label><textarea id="re-steps" rows="5">'+esc(r?(r.steps||[]).join('\n'):'')+'</textarea></div><div class="fam-modal-field"><label>Notities</label><textarea id="re-notes" rows="2">'+esc(r?r.notes||'':'')+'</textarea></div><div style="font-size:12px;color:var(--c-text2,#777)">Zonder eigen foto krijgt dit recept automatisch de FamilyApp Hero voor het gekozen maaltijdtype.</div>',onOpen:function(ctx){var c=ctx.modal.querySelector('#re-cat');c.value=r&&r.cat?r.cat:'Diner';},actions:[{label:'Annuleren'},{label:'Opslaan',primary:true,onClick:function(ctx){var m=ctx.modal,name=(m.querySelector('#re-name').value||'').trim(),ings=(m.querySelector('#re-ings').value||'').split('\n').map(function(x){return x.trim();}).filter(Boolean);if(!name){if(typeof window.showToast==='function')window.showToast('Naam verplicht');return false;}if(!ings.length){if(typeof window.showToast==='function')window.showToast('Voeg minstens 1 ingrediënt toe');return false;}var cat=m.querySelector('#re-cat').value,payload={name:name,cat:cat,cuisine:(m.querySelector('#re-cuis').value||'').trim(),persons:parseInt(m.querySelector('#re-pers').value,10)||4,time:parseInt(m.querySelector('#re-time').value,10)||30,emoji:CAT_ICONS[cat]||'🍴',photo:r?r.photo||null:null,imageMode:r&&r.photo?'custom':'preset',heroPreset:cat.toLowerCase(),ingredients:ings,steps:(m.querySelector('#re-steps').value||'').split('\n').map(function(x){return x.trim();}).filter(Boolean),notes:m.querySelector('#re-notes').value||'',sourceProvider:r&&r.sourceProvider||'manual',sourceUrl:r&&r.sourceUrl||''};if(r)payload.id=r.id;var s=store();if(!s){if(typeof window.showToast==='function')window.showToast('Receptopslag niet beschikbaar');return false;}var job=r?s.upsert(payload):s.create(payload);job.then(function(res){var saved=res.recipe||payload;if(!r&&typeof window.awardXP==='function')window.awardXP(4,'Recept aangemaakt');if(typeof window.showToast==='function')window.showToast('Opgeslagen ✓');setTimeout(function(){renderList();renderDetail(saved.id);},80);}).catch(function(){if(typeof window.showToast==='function')window.showToast('Opslaan mislukt');});return true;}}]});
  }
