'use strict';
// ============================================================
// CLEANING ROUTINE EXPERIENCE v0.3.1
...
(function(){
  ...
  function assignmentPatch(values,existing){
    ...
    if(!selected||selected==='AUTO'){
      patch.assignmentMode='AUTO';patch.preferredAssigneeUid=null;patch.assignmentRequestStatus='AUTO';patch.paused=false;patch.assignmentRequestedByUid=null;patch.assignmentRequestedAt=null;patch.assignmentAcceptedByUid=null;patch.assignmentAcceptedAt=null;return patch;
    }
    if(selected===uid){
      patch.assignmentMode='FIXED_PERSON';patch.preferredAssigneeUid=uid;patch.assignmentRequestStatus='ACCEPTED';patch.paused=false;patch.assignmentAcceptedByUid=uid;patch.assignmentAcceptedAt=Number(existing&&existing.assignmentAcceptedAt)||now();return patch;
    }
    if(existing&&text(existing.preferredAssigneeUid)===selected&&text(existing.assignmentRequestStatus)==='ACCEPTED'){
      patch.assignmentMode='FIXED_PERSON';patch.preferredAssigneeUid=selected;patch.assignmentRequestStatus='ACCEPTED';patch.paused=false;patch.assignmentAcceptedByUid=selected;patch.assignmentAcceptedAt=Number(existing.assignmentAcceptedAt)||now();return patch;
    }
    patch.assignmentMode='REQUESTED';patch.preferredAssigneeUid=selected;patch.assignmentRequestStatus='PENDING';patch.paused=true;patch.assignmentRequestedByUid=uid;patch.assignmentRequestedAt=now();patch.assignmentAcceptedByUid=null;patch.assignmentAcceptedAt=null;return patch;
  }
  ...
  function decorateRoutineRows(){
    ...
    Array.prototype.forEach.call(document.querySelectorAll('#screen-cleaning [data-cleaning-routine-edit]'),function(edit){
      ...
      if(routine.assignmentRequestStatus==='PENDING')label='Verzoek naar '+memberName(routine.preferredAssigneeUid);else if(routine.assignmentRequestStatus==='ACCEPTED'&&routine.preferredAssigneeUid)label='Vast: '+memberName(routine.preferredAssigneeUid);else if(routine.assignmentRequestStatus==='DECLINED')label='Afgewezen · automatisch';
  ...
  function pendingRequests(){var uid=currentUid(),rows=cleaningData().routines||{};return Object.keys(rows).map(function(id){return Object.assign({id:id},rows[id]||{});}).filter(function(routine){return routine&&routine.active!==false&&routine.assignmentRequestStatus==='PENDING'&&text(routine.preferredAssigneeUid)===uid;});}

  function decorateRequests(){
    var panel=document.querySelector('#screen-cleaning .cleaning-panel');if(!panel)return;var inbox=panel.querySelector('[data-cleaning-routine-request-inbox]'),requests=pendingRequests();if(!requests.length){if(inbox)inbox.remove();return;}
  ...
  function resolveRequest(routineId,accept){
    ...
    return write.database.ref(write.path).transaction(function(serverRoot){
      ...
      var timestamp=now();if(accept){routine.assignmentMode='FIXED_PERSON';routine.assignmentRequestStatus='ACCEPTED';routine.assignmentAcceptedAt=timestamp;routine.assignmentAcceptedByUid=write.ctx.uid;routine.paused=false;routine.updatedAt=timestamp;routine.updatedByUid=write.ctx.uid;injectAcceptedRoutine(root,routineId,write.ctx.uid,timestamp);}else{routine.assignmentMode='AUTO';routine.assignmentRequestStatus='DECLINED';routine.assignmentDeclinedAt=timestamp;routine.assignmentDeclinedByUid=write.ctx.uid;routine.preferredAssigneeUid=null;routine.paused=false;routine.updatedAt=timestamp;routine.updatedByUid=write.ctx.uid;}transitionError=null;return root;
