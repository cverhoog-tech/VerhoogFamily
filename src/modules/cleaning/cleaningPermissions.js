...
//
// Adult household members can manage normal Cleaning structure (rooms and
// routines) for every household. Only destructive structure changes and
// household-wide controls stay manager-only.
//
...
(function(){
  ...
  var CAP={
    STRUCTURE:'STRUCTURE',
    DESTRUCTIVE_STRUCTURE:'DESTRUCTIVE_STRUCTURE',
    PLANNING:'PLANNING',
  ...
  function capabilities(value){
    ...
    if(value===ROLE.MEMBER){base[CAP.STRUCTURE]=true;base[CAP.PLANNING]=true;base[CAP.ASSIGNMENTS]=true;base[CAP.AVAILABILITY]=true;base[CAP.SUPPLIES]=true;return base;}
    return base;
  }
  function can(capability){return capabilities()[capability]===true;}
  function permissionMessage(capability){
    if(capability===CAP.STRUCTURE)return'Dit profiel kan geen kamers of schoonmaakroutines beheren.';
    if(capability===CAP.DESTRUCTIVE_STRUCTURE)return'Alleen een beheerder kan kamers of schoonmaakroutines definitief verwijderen.';
  ...
  function installRepositoryGuards(){
    ...
    ['createRoom','updateRoom','createRoutineItem','updateRoutineItem'].forEach(function(name){wrap(repo,name,function(){return can(CAP.STRUCTURE)?true:CAP.STRUCTURE;});});
    ['removeRoom','removeRoutineItem'].forEach(function(name){wrap(repo,name,function(){return can(CAP.DESTRUCTIVE_STRUCTURE)?true:CAP.DESTRUCTIVE_STRUCTURE;});});
  ...
  function ensureNote(screen,currentRole){
    ...
    note.textContent=currentRole===ROLE.LIMITED?'Beperkt profiel · je kunt toegewezen schoonmaakbeurten uitvoeren, verzoeken beantwoorden en hulp vragen.':'Gezinslid · je kunt kamers, routines, planning en voorraad beheren. Definitief verwijderen en huishoudbrede pauzes blijven voor beheerders.';
  }
  function decorateUi(){
    state.queued=false;installGuards();var screen=document.getElementById('screen-cleaning');if(!screen)return;var currentRole=role(),limited=currentRole===ROLE.LIMITED;
    each('#screen-cleaning [data-cleaning-room-add],#screen-cleaning [data-cleaning-room-edit],#screen-cleaning [data-cleaning-routine-add],#screen-cleaning [data-cleaning-routine-edit],#screen-cleaning [data-cleaning-template-add],#screen-cleaning [data-cleaning-room-move]',function(node){markHidden(node,!can(CAP.STRUCTURE));});
    each('#screen-cleaning [data-cleaning-routine-title],#screen-cleaning [data-cleaning-routine-interval],#screen-cleaning [data-cleaning-routine-minutes],#screen-cleaning [data-cleaning-routine-priority]',function(node){markDisabled(node,!can(CAP.STRUCTURE));});
    each('#screen-cleaning [data-cleaning-routine-remove],#screen-cleaning [data-cleaning-room-delete-open],#screen-cleaning [data-cleaning-room-delete-confirm],#screen-cleaning [data-cleaning-routine-delete-open],#screen-cleaning [data-cleaning-routine-delete-confirm]',function(node){markHidden(node,!can(CAP.DESTRUCTIVE_STRUCTURE));});
    ...
    each('#screen-cleaning [data-cleaning-room-pause]',function(node){markHidden(node,!can(CAP.HOUSEHOLD_AVAILABILITY));});
  ...
  function forbiddenClick(event){
    ...
    if(closest('[data-cleaning-room-add],[data-cleaning-room-edit],[data-cleaning-routine-add],[data-cleaning-routine-edit],[data-cleaning-template-add],[data-cleaning-room-move]')&&!can(CAP.STRUCTURE))return CAP.STRUCTURE;
    if(closest('[data-cleaning-routine-remove],[data-cleaning-room-delete-open],[data-cleaning-room-delete-confirm],[data-cleaning-routine-delete-open],[data-cleaning-routine-delete-confirm]')&&!can(CAP.DESTRUCTIVE_STRUCTURE))return CAP.DESTRUCTIVE_STRUCTURE;
  ...
  window.CleaningPermissions={version:VERSION,ROLE:ROLE,CAP:CAP,role:role,roleLabel:roleLabel,capabilities:capabilities,can:can,require:requireCapability,permissionMessage:permissionMessage,currentMember:currentMember,canPauseRoutine:canPauseRoutine,start:start,_normalizeRole:normalizeRole,_installGuards:installGuards,_decorateUi:decorateUi};
  start();
})();
