...
(function(){
  ...
  var VERSION='0.1.0';
  var state={root:{},unsubscribe:null,observer:null,observedOverlay:null,queued:false,roomId:null,targetRoutineId:null,busyLink:null,busyAdd:false,wrapped:false};
...
';document.head.appendChild(style);
  ...
  function decorateRows(overlay){
    ...
    var empty=overlay.querySelector('.cleaning-supply-empty'),emptyText=rows.length?'Nog geen benodigdheden gekoppeld. Voeg hierboven een item toe aan een routine.':'Maak eerst een routine. Daarna kun je hier direct benodigdheden toevoegen.';if(empty&&empty.textContent!==emptyText)empty.textContent=emptyText;
  }
  ...
  function observeOverlay(){
    var overlay=document.getElementById('cleaning-supplies-overlay');if(!overlay||typeof MutationObserver==='undefined'||state.observedOverlay===overlay)return;
    if(state.observer){try{state.observer.disconnect();}catch(error){}}
    state.observer=new MutationObserver(queue);state.observer.observe(overlay,{childList:true,subtree:true});state.observedOverlay=overlay;
  }
  function decorate(){state.queued=false;ensureStyle();wrapOpenRoom();observeOverlay();var overlay=document.getElementById('cleaning-supplies-overlay');if(!overlay||!state.roomId)return;forceAllItems(overlay);decorateDirect(overlay);decorateRows(overlay);}
