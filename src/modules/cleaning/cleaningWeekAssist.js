...
// - lets a user hide supplies they explicitly do not want to buy
// - removes only exact open Cleaning-linked Shopping items when dismissed
// - only adds visible supplies to Shopping after an explicit user action
...
// boundaries only after a user action. The "Niet kopen" preference is a
// per-user/per-household presentation preference in localStorage and never
// mutates routine supplyIds or inventory truth. It never owns Planning copy.
// ============================================================
(function(){
  ...
  var HIDDEN_KEY_PREFIX='familyapp_cleaning_week_supply_hidden_v1_';
  var state={
  ...
  function householdContext(){try{return window.HouseholdContext&&typeof window.HouseholdContext.snapshot==='function'?window.HouseholdContext.snapshot():null;}catch(error){return null;}}
  function hiddenStorageKey(){var ctx=householdContext();return ctx&&ctx.uid&&ctx.householdId?HIDDEN_KEY_PREFIX+ctx.uid+'_'+ctx.householdId:null;}
  function hiddenSupplyIds(){var key=hiddenStorageKey();if(!key||typeof localStorage==='undefined')return{};try{var raw=JSON.parse(localStorage.getItem(key)||'[]'),out={};(Array.isArray(raw)?raw:[]).forEach(function(id){id=text(id);if(id)out[id]=true;});return out;}catch(error){return{};}}
  function saveHiddenSupplyIds(map){var key=hiddenStorageKey();if(!key||typeof localStorage==='undefined')return false;try{var ids=Object.keys(map||{}).filter(function(id){return map[id]===true;}).sort();if(ids.length)localStorage.setItem(key,JSON.stringify(ids));else localStorage.removeItem(key);return true;}catch(error){return false;}}
  function filterHiddenNeeds(needs,hidden){hidden=hidden||{};return(Array.isArray(needs)?needs:[]).filter(function(row){return row&&!hidden[text(row.id)];});}
  function restoreHiddenSupplies(){var key=hiddenStorageKey();if(key&&typeof localStorage!=='undefined')try{localStorage.removeItem(key);}catch(error){}toast('Verborgen benodigdheden hersteld ✓');queue();}
  ...
  function openCleaningShoppingRowsForSupply(root,projection,supplyId){supplyId=text(supplyId);if(!supplyId)return[];return shoppingRows(projection).filter(function(row){var item=row.item;return !!(item&&item.done!==true&&text(item.source).toLowerCase()==='cleaning'&&resolveSupplyId(root,item)===supplyId);});}
  ...
  function model(){
    var root=cleaningRoot(),conflicts=deriveConflicts(root,state.calendar,{days:7}),openLookup=openShoppingLookup(root,state.shopping),allNeeds=weeklySupplyNeeds(root,{days:7}),hidden=hiddenSupplyIds(),needs=filterHiddenNeeds(allNeeds,hidden),hiddenCount=allNeeds.length-needs.length;needs.forEach(function(row){row.onShopping=!!openLookup[row.id];});
    conflicts.forEach(function(row){row.suggestions=suggestTimes(root,row.event,state.calendar,3);});
    return{conflicts:conflicts,needs:needs,hiddenCount:hiddenCount,purchased:purchasedSupplySuggestions(root,state.shopping)};
  }
  ...
  function hiddenResetMarkup(hiddenCount){return hiddenCount?'<button type="button" class="cleaning-week-hidden-reset" data-cleaning-week-restore-hidden>'+hiddenCount+' '+(hiddenCount===1?'verborgen item':'verborgen items')+' · herstellen</button>':'';}
  function needsMarkup(rows,hiddenCount){if(!rows.length)return'<div class="cleaning-week-ok">'+(hiddenCount?'✓ Je hebt alle voorgestelde aankopen verborgen.':'✓ Voor de komende 7 dagen zijn geen gekoppelde benodigdheden met status Bijna op of Op nodig.')+'</div>'+hiddenResetMarkup(hiddenCount);var pending=rows.filter(function(row){return !row.onShopping;});var list='<div class="cleaning-week-list">'+rows.map(function(row){var dismissKey='dismiss:'+row.id;return'<div class="cleaning-week-row"><div class="cleaning-week-row-top"><div><strong>'+esc(row.name)+'</strong><small>'+esc(row.roomNames.join(', ')||'Schoonmaken')+' · nodig '+esc(row.dates.map(dateLabel).join(', '))+'</small></div><div class="cleaning-week-row-actions"><span class="cleaning-week-row-tag">'+(row.onShopping?'OP LIJST':esc(statusLabel(row.status)).toUpperCase())+'</span><button type="button" class="cleaning-week-dismiss" data-cleaning-week-dismiss="'+attr(row.id)+'" aria-label="'+attr(row.name)+' niet kopen" title="Niet kopen"'+(state.busy[dismissKey]?' disabled':'')+'>×</button></div></div></div>';}).join('')+'</div>';var key='shopping:add';return list+(pending.length?'<button type="button" class="cleaning-week-action" data-cleaning-week-add-shopping'+(state.busy[key]?' disabled':'')+'>'+(state.busy[key]?'Toevoegen…':pending.length+' '+(pending.length===1?'benodigdheid':'benodigdheden')+' naar Boodschappen')+'</button>':'')+hiddenResetMarkup(hiddenCount);}
  ...
  function markup(view){return'<div class="cleaning-week-assist" data-cleaning-week-assist><article class="cleaning-week-assist-card"><div class="cleaning-week-assist-head"><div><strong>Planningcheck</strong><span>FamilyApp vergelijkt geplande schoonmaakmomenten met jullie Agenda. Er wordt nooit automatisch verplaatst.</span></div><span class="cleaning-week-badge">7 DAGEN</span></div>'+conflictMarkup(view.conflicts)+'</article><article class="cleaning-week-assist-card"><div class="cleaning-week-assist-head"><div><strong>Weekvoorraad</strong><span>Alleen benodigdheden die deze week echt nodig zijn én Bijna op/Op staan. Tik × als je iets niet wilt kopen.</span></div><span class="cleaning-week-badge">SLIM BUNDELEN</span></div>'+needsMarkup(view.needs,view.hiddenCount)+'</article>'+purchasedMarkup(view.purchased)+'</div>';}
  function signature(view){return JSON.stringify({conflicts:view.conflicts.map(function(row){return[row.id,row.date,row.time,row.potential,row.conflicts.map(function(hit){return[hit.id,hit.time,hit.potential];}),row.suggestions];}),needs:view.needs.map(function(row){return[row.id,row.status,row.onShopping,row.dates];}),hiddenCount:view.hiddenCount,purchased:view.purchased.map(function(row){return[row.id,row.status,row.itemUpdated];}),busy:Object.keys(state.busy).sort()});}
  ...
  function addWeekSupplies(){var key='shopping:add';if(state.busy[key])return;var root=cleaningRoot(),open=openShoppingLookup(root,state.shopping),hidden=hiddenSupplyIds(),needs=filterHiddenNeeds(weeklySupplyNeeds(root,{days:7}),hidden).filter(function(row){return !open[row.id];}),records=shoppingRecordsForNeeds(needs),store=window.ShoppingListStore;if(!records.length)return;if(!store||typeof store.addItems!=='function'){toast('Boodschappenlijst is nog niet beschikbaar');return;}state.busy[key]=true;queue();Promise.resolve(store.addItems(null,records,{dedupe:true})).then(function(result){var added=result&&Array.isArray(result.added)?result.added.length:0,skipped=result&&Array.isArray(result.skipped)?result.skipped.length:0;if(added)toast(added+' '+(added===1?'benodigdheid toegevoegd':'benodigdheden toegevoegd')+' ✓');else if(skipped)toast('Benodigdheden staan al op de lijst ✓');else toast('Geen benodigdheden toegevoegd');}).catch(function(error){toast((error&&error.message)||'Toevoegen aan Boodschappen mislukt');}).finally(function(){delete state.busy[key];queue();});}
  function hideSupply(id){
    id=text(id);var key='dismiss:'+id;if(!id||state.busy[key])return;var hidden=hiddenSupplyIds();hidden[id]=true;saveHiddenSupplyIds(hidden);var root=cleaningRoot(),rows=openCleaningShoppingRowsForSupply(root,state.shopping,id),repo=shoppingRepository();
    if(!rows.length){toast('Niet kopen · verborgen uit Weekvoorraad');queue();return;}
    if(!repo||typeof repo.deleteItem!=='function'){toast('Verborgen uit Weekvoorraad · verwijder het item eventueel ook bij Boodschappen');queue();return;}
    state.busy[key]=true;queue();Promise.all(rows.map(function(row){return repo.deleteItem(row.scope,row.listId,row.key);})).then(function(){toast('Niet kopen · ook verwijderd uit Boodschappen ✓');}).catch(function(){toast('Verborgen uit Weekvoorraad · Boodschappen kon niet volledig worden bijgewerkt');}).finally(function(){delete state.busy[key];queue();});
  }
  ...
  function onClick(event){var target=event.target&&event.target.closest?event.target:null;if(!target)return;var dismiss=target.closest('[data-cleaning-week-dismiss]');if(dismiss){event.preventDefault();hideSupply(dismiss.getAttribute('data-cleaning-week-dismiss'));return;}if(target.closest('[data-cleaning-week-restore-hidden]')){event.preventDefault();restoreHiddenSupplies();return;}var move=target.closest('[data-cleaning-week-move]');if(move){event.preventDefault();moveCleaning(move);return;}if(target.closest('[data-cleaning-week-add-shopping]')){event.preventDefault();addWeekSupplies();return;}var restock=target.closest('[data-cleaning-week-restock]');if(restock){event.preventDefault();restockSupply(restock.getAttribute('data-cleaning-week-restock'));}}
  ...
  window.CleaningWeekAssist={
    ...
    _filterHiddenNeeds:filterHiddenNeeds,_hiddenSupplyIds:hiddenSupplyIds,
    _openCleaningShoppingRowsForSupply:openCleaningShoppingRowsForSupply,
