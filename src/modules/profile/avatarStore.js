...
// Base URL pointing at the avatars in this repository.  When new avatar
// images are added to `src/assets/avatars` the URLs below will point at
// those files on GitHub via the raw CDN.  Without this override the app
// would continue to look up images in the nested `FamilyApp-clean-avatar-
// background-fix` folder which no longer exists in the cleaned repo.
const uploadedAvatarBaseUrl =
  'https://raw.githubusercontent.com/cverhoog-tech/VerhoogFamily/main/src/assets/avatars';

// Rarity for the 50 new avatars.  The order here must match the numeric
// ordering of the filenames (avatar‑01.webp … avatar‑50.webp).  See
// `src/assets/avatars` for the actual images.
...
// Static avatar definitions.  Each entry defines a preset avatar
// (category, label, seed/background or explicit URL) that ships with the
// application.  The dicebear entries generate SVG avatars on the fly.
export const animeAvatarCollection = [
  {
    id: 'shadow-swordsman',
    category: 'Helden',
    label: 'Shadow Swordsman',
    seed: 'shadow-swordsman',
    bg: 'e8f5e3',
    rarity: 'legendary',
  },
  {
...
export function avatarUrlForId(id) {
  const found =
    animeAvatarCollection.find((avatar) => avatar.id === id) ||
    animeAvatarCollection[0];
...
export function setPresetAvatar(id) {
  ...
  window.dispatchEvent(
    new CustomEvent('familyapp:avatar-updated', {
      detail: { id, url: avatarUrlForId(id) },
    }),
  );
}
...
export function getUserAvatar(author, explicitAvatar) {
  ...
  if (
    explicitAvatar &&
    (explicitAvatar.includes('dicebear') ||
      explicitAvatar.includes('images.unsplash.com') ||
      explicitAvatar.includes('raw.githubusercontent.com'))
  )
