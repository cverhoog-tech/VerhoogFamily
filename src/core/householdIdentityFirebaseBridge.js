...
(function(){
  ...
  function projectCurrentMember(member,uid){
    if(!member||!uid)return;
    try{
      var avatar=member.avatar||member.avatarUrl||member.photoURL||'';
      if(avatar)localStorage.setItem(scoped('familyapp-current-user-avatar-v2',uid),String(avatar));
      var name=member.displayName||member.name||'';
      if(name&&!localStorage.getItem(scoped('familyapp-profile-name-v2',uid)))localStorage.setItem(scoped('familyapp-profile-name-v2',uid),String(name));
    }catch(e){}
  }
  ...
  function apply(){
    ...
    if(window.HouseholdIdentity&&typeof HouseholdIdentity.saveMembers==='function'&&members.length){
      ...
      if(ctx.uid&&typeof HouseholdIdentity.setActiveMember==='function'){
        ...
        if(mine){
          ...
          projectCurrentMember(mine,ctx.uid);
    ...
    notify(members);
    try{window.dispatchEvent(new CustomEvent('familyapp:household-identity-synced',{detail:{householdId:currentHouseholdId,uid:ctx.uid||null,members:members}}));}catch(e){}
    return members.length>0;
  }

  function scopedOwnProfile(uid){
    var r={};
    if(!uid)return r;
    try{
      var n=localStorage.getItem(scoped('familyapp-profile-name-v2',uid));
      var a=localStorage.getItem(scoped('familyapp-current-user-avatar-v2',uid));
      if(n&&n.trim())r.name=n.trim();
      if(a)r.avatar=a;
    }catch(e){}
    return r;
  }
  function migrationKey(hid,uid){return 'familyapp-firebase-member-profile-migrated-v3:'+hid+':'+uid;}
  ...
  function migrateOwnScopedProfileOnce(){
    ...
    var local=scopedOwnProfile(ctx.uid),server=(lastMembers&&lastMembers[ctx.uid])||{},patch={};
    if(!server.name&&local.name)patch.name=local.name;
    if(!(server.avatar||server.avatarUrl||server.photoURL)&&local.avatar)patch.avatar=local.avatar;
    if(!Object.keys(patch).length){try{localStorage.setItem(key,'1');}catch(e){}return;}
    migrationInFlight=true;updateOwnMemberProfile(patch).then(function(){try{localStorage.setItem(key,'1');}catch(e){}}).catch(function(err){console.warn('[HouseholdIdentityFirebaseBridge] scoped profile migration failed',err);}).finally(function(){migrationInFlight=false;});
  }
  ...
  function attach(ctx){
    ...
    membersCb=function(snapshot){lastMembers=snapshot.val()||{};apply();migrateOwnScopedProfileOnce();};
  ...
  window.addEventListener('familyapp:avatar-updated',function(e){
    var detail=(e&&e.detail)||{},url=detail.url||'',ctx=context();
    if(!url||!ctx.ready)return;
    if(detail.uid&&String(detail.uid)!==String(ctx.uid))return;
