...
(function(){
  ...
  // Asset metadata, not user-specific rendering hacks. The safe region describes
  // the upper-face/head area that should remain visible when an asset is used in
  // a cropped hero presentation. Portrait fallback itself never crops.
  var BUNDLED={
  ...
  function explicitHeroUrl(identityHero,recordHero,identity,record){return firstValue([identityHero,recordHero,identity,record],['url','heroImage','heroUrl'])||'';}
  ...
  function resolveHeroMedia(identity,record,fallbackUrl){
    ...
    if(usesPortraitFallback){
      // Hard invariant: an avatar fallback is never treated as a landscape hero.
      // It must remain fully visible so eyes/face cannot disappear through crop.
