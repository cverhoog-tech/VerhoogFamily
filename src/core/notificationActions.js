'use strict';
// ============================================================
// NOTIFICATION ACTIONS v1.0.0
// One action router for actionable in-app notification events.
// Keeps presentation (toast/notification center) separate from mutations.
// ============================================================
(function(){
  ...
  function acceptTaskHelp(event){
    ...
    if(!exists){
      var m=member(me)||{};
      helpers.push({uid:me,memberId:me,name:m.displayName||m.name||(window.myName||'Gezinslid'),initials:String(m.displayName||m.name||window.myName||'G').slice(0,2).toUpperCase(),joinedAt:Date.now()});
    }
  ...
  function installCenterClicks(){
    document.addEventListener('click',function(ev){
      ...
      if(!row)return;
      var event=byId(row.getAttribute('data-notif-id'));
      if(!isActionable(event))return;
      ev.preventDefault();ev.stopPropagation();if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();
      run(event).then(function(){if(typeof window.renderNotifs==='function')window.renderNotifs();}).catch(function(err){console.warn('[NotificationActions]',err);if(typeof window.showToast==='function')window.showToast('Hulp accepteren mislukt');});
    },true);
  }
