...
(function(){
  ...
  function onlineReady(scope){return !!(db()&&uid()&&(scope==='private'||familyId())&&!window.offlineMode);}
  function cacheKey(scope,c){var owner=scope==='shared'?(familyId()||'unresolved-household'):(uid()||'unresolved-user');return CACHE_PREFIX+scope+'_'+owner+'_'+c;}
  function readCache(scope,c,f){try{return parse(localStorage.getItem(cacheKey(scope,c)),f);}catch(e){return f;}}
  function writeCache(scope,c,v){try{localStorage.setItem(cacheKey(scope,c),JSON.stringify(v));}catch(e){}return v;}
  function emit(scope,c,v,source){var k=scope+':'+c,p={scope:scope,collection:c,value:v,source:source||'local',at:now()};(listeners[k]||[]).slice().forEach(function(fn){try{fn(p);}catch(e){}});try{window.dispatchEvent(new CustomEvent('familyapp:data:'+k,{detail:p}));}catch(e){}}
  function on(scope,c,cb){var k=scope+':'+c;if(!listeners[k])listeners[k]=[];listeners[k].push(cb);return function(){listeners[k]=(listeners[k]||[]).filter(function(fn){return fn!==cb;});};}
  function rootRef(scope,c){var d=db();if(!d)return null;if(scope==='shared'){var f=familyId();return f?d.ref('families/'+f+'/shared/'+c):null;}var u=uid();return u?d.ref('users/'+u+'/private/'+c):null;}
  function clone(v){if(v===undefined)return undefined;return JSON.parse(JSON.stringify(v));}
  function getPath(obj,path,f){var cur=obj;for(var i=0;i<path.length;i++){if(cur==null||cur[path[i]]===undefined)return f;cur=cur[path[i]];}return cur;}
  function setPath(obj,path,value){var out=obj&&typeof obj==='object'?clone(obj):{},cur=out;for(var i=0;i<path.length-1;i++){if(!cur[path[i]]||typeof cur[path[i]]!=='object')cur[path[i]]={};cur=cur[path[i]];}if(value==null)delete cur[path[path.length-1]];else cur[path[path.length-1]]=value;return out;}
  ...
  function defaultShoppingList(items){return{id:'household_default',name:'Gezinslijst',icon:'🛒',category:'groceries',visibility:'household',ownerUid:uid(),createdBy:uid(),createdAt:now(),updatedAt:now(),items:itemMap(items)};}
  function migrateLegacyShopping(){if(!uid()||!familyId())return Promise.resolve({migrated:false,reason:'identity-unresolved'});var legacy=Array.isArray(window.shopData)?window.shopData.slice():[];return read('shared',SHARED.shoppingLists,{}).then(function(existing){if(existing&&Object.keys(existing).length)return{migrated:false,reason:'already-exists'};if(!legacy.length)return{migrated:false,reason:'no-legacy-items'};var list=defaultShoppingList(legacy);return writePath('shared',SHARED.shoppingLists,[list.id],list).then(function(result){return{migrated:true,list:list,result:result};});});}
  function status(){return{version:VERSION,onlineReady:onlineReady('shared'),privateReady:onlineReady('private'),userId:uid(),familyId:familyId(),pendingWrites:pending().length};}
  window.FamilyDataStore={version:VERSION,sharedCollections:SHARED,privateCollections:PRIVATE,status:status,makeId:makeId,itemMap:itemMap,defaultShoppingList:defaultShoppingList,migrateLegacyShopping:migrateLegacyShopping,flushPending:flushPending,readShared:function(c,f){return read('shared',c,f);},readPrivate:function(c,f){return read('private',c,f);},writeShared:function(c,v,o){return write('shared',c,v,o);},writePrivate:function(c,v,o){return write('private',c,v,o);},subscribeShared:function(c,cb,f){return subscribe('shared',c,cb,f);},subscribePrivate:function(c,cb,f){return subscribe('private',c,cb,f);},writeSharedRecord:function(c,id,v,o){return writePath('shared',c,[id],v,o);},writePrivateRecord:function(c,id,v,o){return writePath('private',c,[id],v,o);},mutateSharedRecord:function(c,id,u,f){return transactPath('shared',c,[id],u,f);},mutatePrivateRecord:function(c,id,u,f){return transactPath('private',c,[id],u,f);},writeSharedPath:function(c,path,v,o){return writePath('shared',c,path,v,o);},writePrivatePath:function(c,path,v,o){return writePath('private',c,path,v,o);},transactSharedPath:function(c,path,u,f){return transactPath('shared',c,path,u,f);},transactPrivatePath:function(c,path,u,f){return transactPath('private',c,path,u,f);}};
