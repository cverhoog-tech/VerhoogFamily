...
(function(){
  ...
  function showError(err){
    var b=button();if(b){b.innerHTML=buttonHtml();b.disabled=false;}
    var code=err&&err.code||'';
    var msg=tr('auth.apple.failed','Apple-login mislukt. Probeer opnieuw.');
    if(code==='auth/popup-closed-by-user'||code==='auth/cancelled-popup-request')msg=tr('auth.apple.cancelled','Apple-login is geannuleerd. Probeer opnieuw.');
    else if(code==='auth/popup-blocked')msg=tr('auth.apple.popupBlocked','Safari kon het Apple-inlogvenster niet openen. Tik nogmaals op “Inloggen met Apple”.');
    else if(code==='auth/unauthorized-domain')msg=tr('auth.domainUnauthorized','Dit domein is nog niet toegestaan voor Firebase Authentication.');
    else if(code==='auth/operation-not-allowed')msg=tr('auth.apple.notEnabled','Apple-login is nog niet geactiveerd in Firebase Authentication.');
    else if(code==='auth/account-exists-with-different-credential')msg=tr('auth.accountExistsOtherProvider','Er bestaat al een account met hetzelfde e-mailadres. Log eerst in met je bestaande methode; daarna kunnen we deze methode veilig koppelen.');
  ...
  function provider(){
    if(!window.firebase||!firebase.auth||!firebase.auth.OAuthProvider)throw new Error(tr('auth.firebaseAppleUnavailable','Firebase Apple OAuth provider is niet beschikbaar'));
    var p=new firebase.auth.OAuthProvider('apple.com');
    p.addScope('email');
    p.addScope('name');
    p.setCustomParameters({locale:localeParam()});
    return p;
  }
  function ensureButton(){
    ...
    b.id=BUTTON_ID;b.type='button';b.setAttribute('aria-label',tr('auth.apple.button','Inloggen met Apple'));
  ...
  function signIn(){
    if(!enabled())return;
    var a=auth();if(!a){showError(new Error(tr('auth.firebaseNotReady','Firebase is nog niet klaar. Probeer opnieuw.')));return;}
    clearError();
    var b=ensureButton();if(b){b.textContent=tr('auth.apple.opening','Apple openen...');b.disabled=true;}
    var p;try{
      var pr=provider();
  ...
  window.addEventListener('familyapp:language-changed',function(){var b=button();if(b&&!b.disabled){b.setAttribute('aria-label',tr('auth.apple.button','Inloggen met Apple'));b.innerHTML=buttonHtml();}});
