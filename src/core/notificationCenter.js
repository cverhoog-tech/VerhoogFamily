...
(function(){
  ...
  function itemHtml(n){
    ...
    var pillCls=pillClassFor(state),actor=n.actor&&n.actor.name?esc(n.actor.name)+' · ':'',time=n.createdAt?new Date(n.createdAt).toLocaleString('nl-NL',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):'';
    return '<article class="nc-item'+(read?' is-read':'')+'" data-notif-id="'+esc(n.id)+'" tabindex="0" role="button"><div class="nc-icon" style="background:'+esc(n.bg||'#ede9fe')+'">'+iconHtml(n)+'</div><div class="nc-copy"><div class="nc-title">'+esc(n.title||'Melding')+'</div><div class="nc-body">'+esc(n.body||'')+'</div>'+(state.statusLabel?'<span class="nc-pill'+(pillCls?' '+pillCls:'')+'">'+esc(state.statusLabel)+'</span>':'')+'<div class="nc-meta">'+actor+esc(time)+'</div></div>'+(!read?'<span class="nc-dot"></span>':'')+'</article>';
  }
  function render(){ensureStyles();var el=document.getElementById('notif-list');if(!el)return;if(!window.NotificationStore){el.innerHTML='<div style="padding:30px;text-align:center;color:var(--c-text2)">Meldingen worden geladen…</div>';return;}NotificationStore.ensureSubscription();var items=NotificationStore.list();el.innerHTML=items.map(itemHtml).join('')||'<div style="padding:30px;text-align:center;color:var(--c-text2)">Geen meldingen</div>';var dot=document.getElementById('notif-dot');if(dot)dot.style.display=NotificationStore.unreadCount()?'block':'none';}
  ...
  function install(){
    ...
    window.addEventListener('familyapp:notifications-changed',syncPresentation);window.addEventListener('familyapp:party-quests-updated',syncPresentation);window.addEventListener('familyapp:tasks-updated',syncPresentation);
