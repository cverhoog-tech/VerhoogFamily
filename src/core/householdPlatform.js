...
(function(){
  ...
  function createInvite(role){
    ...
    return d.ref('families/'+hid+'/members/'+u.uid).once('value').then(function(ms){var m=ms.val();if(!m||m.status!=='active'||(m.role!=='owner'&&m.role!=='admin'))throw new Error('Alleen een beheerder kan uitnodigen');var attempt=0;function reserve(){if(++attempt>5)throw new Error('Kon geen uitnodigingscode maken');var code=slugCode();return d.ref('invites/'+code).transaction(function(cur){if(cur)return;return{code:code,householdId:hid,createdBy:u.uid,role:role,status:'active',createdAt:now(),expiresAt:now()+INVITE_TTL,maxUses:1,uses:0};}).then(function(r){return r.committed?code:reserve();});}return reserve();});
  }
  ...
  function joinHousehold(code){
    ...
    return inspectInvite(code).then(function(info){var inv=info.invite,hid=inv.householdId,member=memberRecord(u,inv.role||'adult');
      return d.ref('invites/'+code).transaction(function(cur){if(!cur||cur.status!=='active'||cur.expiresAt<now()||(cur.uses||0)>=(cur.maxUses||1))return;cur.uses=(cur.uses||0)+1;if(cur.uses>=cur.maxUses)cur.status='used';cur.usedBy=u.uid;cur.usedAt=now();return cur;}).then(function(r){
        if(!r.committed)throw new Error('Deze uitnodiging is al gebruikt');
        var claim={code:code,householdId:hid,uid:u.uid,role:inv.role||'adult',status:'approved',createdAt:now(),expiresAt:now()+CLAIM_TTL};
        ...
        }).then(function(){return d.ref('joinClaims/'+hid+'/'+u.uid).remove().catch(function(){});}).then(function(){setGlobals(hid,member.name);startPresence(hid);return info;});
      });
    });
  }
