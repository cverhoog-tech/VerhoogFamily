...
(function(){
  ...
  function normalizeMap(value){
    ...
    if(Array.isArray(value)){
      value.forEach(function(item){if(item&&item.id)out[item.id]=item;});
      return out;
    }
    Object.keys(value).forEach(function(key){if(value[key])out[key]=Object.assign({id:key},value[key]);});
    return out;
  }
  ...
  function canSee(event){
    ...
    if(a.kind==='uids')return Array.isArray(a.uids)&&a.uids.indexOf(id)>-1;
    return false;
  }
  function sortedVisible(){
    return Object.keys(records).map(function(k){return records[k];}).filter(canSee).sort(function(a,b){
      var d=(Number(b.createdAt)||0)-(Number(a.createdAt)||0);
      if(d)return d;
  ...
  function formatRelative(ts){
    ...
    try{return new Date(ts).toLocaleDateString('nl-NL',{day:'numeric',month:'short'});}catch(e){return'';}
  }
  ...
  function mirrorLegacy(){
    // Compatibility mirror only. NotificationStore remains authoritative.
    window.notifData=sortedVisible().map(legacyProjection);
  ...
  function emit(meta){
    ...
    listeners.slice().forEach(function(fn){try{fn(list,meta||{});}catch(e){console.error('[NotificationStore listener]',e);}});
    try{window.dispatchEvent(new CustomEvent('familyapp:notifications-changed',{detail:{items:list,meta:meta||{},unread:unreadCount()}}));}catch(e){}
  }
  function detectIncoming(previous){
    var me=uid(),fresh=[];
    if(!me)return fresh;
    Object.keys(records).forEach(function(id){
      var e=records[id];
      if(previous[id]||!canSee(e)||isRead(e))return;
      if(e.actor&&e.actor.uid===me)return;
      fresh.push(e);
    });
  ...
  function ensureSubscription(){
    ...
    unsubscribe=FamilyDataStore.subscribeShared(COLLECTION,function(value,meta){
      ...
      var incoming=detectIncoming(previous);
      emit({source:meta&&meta.source||'shared',incoming:incoming});
      incoming.forEach(function(event){
        try{window.dispatchEvent(new CustomEvent('familyapp:notification-received',{detail:{event:event}}));}catch(e){}
      });
    },{});
    return true;
  }
  ...
  function publish(input){
    validate(input);
    if(!window.FamilyDataStore)return Promise.reject(new Error('FamilyDataStore niet beschikbaar'));
    ensureSubscription();
    ...
    var event={
      ...
      audience:input.audience||audienceSelf(),
      entity:input.entity||null,
      data:input.data||{},
      channels:Array.isArray(input.channels)?input.channels.slice():['inApp'],
    ...
    records[id]=event;
    emit({source:'local-publish',event:event});
  ...
  function legacy(icon,bg,title,body){
    // Existing addNotif callers are intentionally self-scoped. Cross-member
    // delivery must be explicit through typed domain events, never implicit.
    return publishSelf('system.message',{icon:icon||'🔔',bg:bg||'#ede9fe',title:title||'Melding',body:body||'',data:{legacy:true}});
  }
  ...
  function markRead(id){
    var me=uid();if(!me||!records[id]||!window.FamilyDataStore)return Promise.resolve(false);
    records[id].readBy=Object.assign({},records[id].readBy||{});records[id].readBy[me]=now();records[id].updatedAt=now();emit({source:'local-read',id:id});
    return FamilyDataStore.writeSharedPath(COLLECTION,[id,'readBy',me],records[id].readBy[me]).then(function(){return true;});
  }
  function markAllRead(){
    var unread=sortedVisible().filter(function(e){return!isRead(e);});
    return unread.reduce(function(chain,e){return chain.then(function(){return markRead(e.id);});},Promise.resolve()).then(function(){return unread.length;});
  }
  function dismiss(id){
    var me=uid();if(!me||!records[id]||!window.FamilyDataStore)return Promise.resolve(false);
    records[id].dismissedBy=Object.assign({},records[id].dismissedBy||{});records[id].dismissedBy[me]=now();records[id].updatedAt=now();emit({source:'local-dismiss',id:id});
    return FamilyDataStore.writeSharedPath(COLLECTION,[id,'dismissedBy',me],records[id].dismissedBy[me]).then(function(){return true;});
  }
  function clearVisible(){
    var visible=sortedVisible();
    return visible.reduce(function(chain,e){return chain.then(function(){return dismiss(e.id);});},Promise.resolve()).then(function(){return visible.length;});
  }
