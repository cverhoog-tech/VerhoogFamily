'use strict';
// ============================================================
// NOTIFICATION DELIVERY v1.0.0
// Presentation/channel subscriber for incoming NotificationStore events.
// Never creates or mutates domain events; no write-back loops.
// ============================================================
(function(){
  ...
  function ensureStyles(){
    ...
    var s=document.createElement('style');s.id='notification-delivery-style';s.textContent=[
      '.notif-live-banner{position:fixed;top:max(64px,calc(env(safe-area-inset-top) + 52px));left:50%;transform:translateX(-50%);z-index:12020;width:min(420px,calc(100% - 24px));background:var(--c-surface);border:1px solid var(--c-border);border-radius:18px;padding:12px 13px;box-shadow:0 14px 40px rgba(15,23,42,.18);display:flex;align-items:center;gap:11px;animation:notifLiveIn .22s ease-out}',
  ...
  function next(){
    ...
    el.innerHTML='<div class="notif-live-icon" style="background:'+esc(event.bg||'#ede9fe')+'">'+esc(event.icon||'🔔')+'</div>'
      ...
      +(event.actor&&event.actor.name?'<div class="notif-live-from">Van '+esc(event.actor.name)+'</div>':'')+'</div>'
    ...
    // Transient UI timeout only; this is not dependency polling or state synchronization.
