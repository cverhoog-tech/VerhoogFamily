'use strict';
// ============================================================
// NOTIFICATION DELIVERY v1.1.0
// Presentation/channel subscriber for incoming NotificationStore events.
// Actionable events delegate to NotificationActions; ordinary events open the
// notification center. Domain writes never live in this presentation layer.
// ============================================================
(function(){
  ...
  function ensureStyles(){
    ...
    var s=document.createElement('style');s.id='notification-delivery-style';s.textContent=[
      '.notif-live-banner{position:fixed;top:max(64px,calc(env(safe-area-inset-top) + 52px));left:50%;transform:translateX(-50%);z-index:12020;width:min(420px,calc(100% - 24px));background:var(--c-surface);border:1px solid var(--c-border);border-radius:18px;padding:12px 13px;box-shadow:0 14px 40px rgba(15,23,42,.18);display:flex;align-items:center;gap:11px;animation:notifLiveIn .22s ease-out;cursor:pointer}',
      ...
      '.notif-live-action{font-size:10px;font-weight:900;color:var(--c-primary);margin-top:4px}',
      '.notif-live-close{border:0;background:var(--c-surface2);color:var(--c-text2);width:28px;height:28px;border-radius:9px;display:grid;place-items:center;font-size:13px;flex:0 0 auto}',
  ...
  function next(){
    ...
    el.innerHTML='<div class="notif-live-icon" style="background:'+esc(event.bg||'#ede9fe')+'">'+esc(event.icon||'🔔')+'</div>'
      ...
      +(isAction?'<div class="notif-live-action">Tik om te accepteren →</div>':'')+'</div>'
    ...
    el.querySelector('.notif-live-close').onclick=function(ev){ev.stopPropagation();close();};
    el.onclick=function(){
      if(actionable(event)){
        NotificationActions.run(event).catch(function(err){console.warn('[NotificationDelivery]',err);if(typeof window.showToast==='function')window.showToast('Actie uitvoeren mislukt');});
        close();return;
      }
      if(window.NotificationStore&&event.id)NotificationStore.markRead(event.id);
    ...
    setTimeout(function(){if(activeBanner===el)close();},6500);
  }
  function receive(event){if(!event)return;queue.push(event);next();}
  function install(){
    ensureStyles();
    if(window.__notificationDeliveryListener)return;
    window.__notificationDeliveryListener=true;
