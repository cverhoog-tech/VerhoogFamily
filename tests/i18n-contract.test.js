...
(function () {
  ...
  const english = loadI18n({ store: { 'familyapp-language-v1': 'en' } });
  assert.strictEqual(english.i18n.translateUiText('Opslaan'), 'Save');
  assert.strictEqual(english.i18n.translateUiText('Kamer toevoegen'), 'Add room');
  assert.strictEqual(english.i18n.translateUiText('Nog geen transacties'), 'No transactions yet');

  const turkish = loadI18n({ store: { 'familyapp-language-v1': 'tr' } });
  assert.strictEqual(turkish.i18n.translateUiText('Uitloggen'), 'Çıkış yap');
  assert.strictEqual(turkish.i18n.translateUiText('3 taken'), '3 görev');

  const polishUi = loadI18n({ store: { 'familyapp-language-v1': 'pl' } });
  assert.strictEqual(polishUi.i18n.translateUiText('5 taken'), '5 zadań');
  assert.strictEqual(polishUi.i18n.translateUiText('Routine toevoegen'), 'Dodaj rutynę');
