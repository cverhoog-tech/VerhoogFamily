...
function modal(html){
  ...
  A('.fqClose',m).forEach(function(b){b.onclick=function(){closeModal();};});
  return m;
}
function closeModal(){
  var m=document.getElementById('fqModal');
...
function hero(img,badges,title,meta){
  return '<div class="fqPage"><button class="fqClose">✕</button>'+
...
function detail(id){
  ...
  var m=modal(
    ...
    '<div class="fqBox">'+
      '<b>'+L('rewards')+'</b>'+
      '<div class="fqRewards">'+
        '<div class="fqRewardChip"><div class="fqRewardIcon">🏆</div><div class="fqRewardLabel">XP</div><div class="fqRewardVal">'+x[6]+'</div></div>'+
        '<div class="fqRewardChip"><div class="fqRewardIcon">⭐</div><div class="fqRewardLabel">Discipline</div><div class="fqRewardVal">+5</div></div>'+
        '<div class="fqRewardChip"><div class="fqRewardIcon">🏠</div><div class="fqRewardLabel">Homekeeping</div><div class="fqRewardVal">+10</div></div>'+
      '</div>'+
    '</div>'+
    '</div>'+
...
function showGQPopup(questTitle){
  ...
  pop.innerHTML=
    '<button class="fqGQClose">✕</button>'+
    '<div class="fqGQTag">'+L('groupq')+'</div>'+
    '<div class="fqGQTitle">'+L('helpreq')+'</div>'+
    '<div style="font-size:13px;color:var(--m);margin-bottom:4px;">Shane '+L('helpat')+'</div>'+
    '<div class="fqGQQuest">'+questTitle+'</div>'+
    '<div class="fqGQAvs">'+
      '<div class="fqPav fqPav3" style="width:32px;height:32px;font-size:11px;">SH</div>'+
      '<div class="fqPav fqPav1" style="width:32px;height:32px;font-size:11px;">SK</div>'+
  ...
  A('.fqGQClose,.fqGQBtn',pop).forEach(function(b){
    b.onclick=function(){pop.classList.remove('open');};
  });
}
...
function create(){
  var m=modal(
    '<div class="fqPage"><button class="fqClose">✕</button>'+
    '<div class="fqContent fqForm">'+
    '<h2 style="color:#fff;font-size:26px;font-weight:950;margin-bottom:16px;">'+L('newq')+'</h2>'+
    '<label>Naam</label><input id="qn" placeholder="Wat moet er gebeuren?">'+
    '<label>Omschrijving</label><textarea id="qd" rows="3" placeholder="Korte beschrijving..."></textarea>'+
    '<label>Quest type</label>'+
    '<div class="fqSeg"><button id="qside" class="on">Side Quest</button><button id="qdun">Dungeon</button><button id="qraid">Raid</button></div>'+
    '<label>Prioriteit</label>'+
    '<div class="fqSeg"><button id="qlaag" class="on">Laag</button><button id="qnorm">Normaal</button><button id="qhoog">Hoog</button></div>'+
    '<label>Datum</label><input id="qdate" type="date">'+
    '<label>Toegewezen aan</label>'+
    '<select id="qwho"><option>Shane</option><option>Esra</option><option>Beiden</option></select>'+
    '</div>'+
    '<div class="fqDoneWrap"><button class="fqDone fqSave">Quest opslaan</button></div>'+
    '</div>'
  );

  var qtype='SIDE QUEST',qprio='laag';

  function onType(id,t){A('#qside,#qdun,#qraid',m).forEach(function(b){b.classList.remove('on');});A('#'+id,m)[0].classList.add('on');qtype=t;}
  function onPrio(id,p){A('#qlaag,#qnorm,#qhoog',m).forEach(function(b){b.classList.remove('on');});A('#'+id,m)[0].classList.add('on');qprio=p;}

  A('#qside',m)[0].onclick=function(){onType('qside','SIDE QUEST');};
  A('#qdun',m)[0].onclick=function(){onType('qdun','DUNGEON');};
  A('#qraid',m)[0].onclick=function(){onType('qraid','RAID');};
  A('#qlaag',m)[0].onclick=function(){onPrio('qlaag','laag');};
  A('#qnorm',m)[0].onclick=function(){onPrio('qnorm','normaal');};
  A('#qhoog',m)[0].onclick=function(){onPrio('qhoog','hoog');};

  A('.fqSave',m)[0].onclick=function(){
    ...
    var xp=xpForType(qtype,qprio);
    data.push(['q'+Date.now(),qtype,n,d,date,who,xp,pick(n+' '+d),['Eerste stap'],0,'once',date,qprio]);
    save();
    closeModal();
    var r=root();if(r){r.dataset.v023='';render(true);}
  };
...
function langBtn(){
  ...
  b.onclick=function(){lang=lang==='en'?'nl':'en';localStorage.setItem('fam_lang',lang);location.reload();};
...
/* ── MAIN LOOP ── */
function run(){home();bind();render(false);langBtn();if(!task()){document.body.classList.remove('famTask');}}
