...
assert(shell.includes('/src/styles/tasksV3Polish.css?v=2'),'canonical v7 shell must serve current V3 reference polish');
...
assert(shell.includes('/src/modules/tasks/v3/taskOverviewV3.js?v=3'),'canonical v7 shell must serve responsive V3 overview');
assert(shell.includes('/src/modules/tasks/v3/taskDetailV3.js?v=3'),'canonical v7 shell must serve responsive V3 detail');
...
assert(overview.includes("version:'3.1.0'"),'responsive overview V3 identity missing');
assert(detail.includes("var VERSION='3.1.0'"),'responsive detail V3 identity missing');
assert(detail.includes('applySubtaskState'),'subtasks must paint locally without rebuilding the hero');
assert(detail.includes('state.subtaskPending'),'detail must guard optimistic subtask writes');
assert(polish.includes('aspect-ratio:1/1!important'),'round task/subtask control guard missing');
assert(polish.includes('touch-action:manipulation'),'fast mobile checkbox tap guard missing');
...
console.log('Tasks V3 canonical shell / room photography / responsive checkbox contract: PASS');