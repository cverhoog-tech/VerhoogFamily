...
(async function(){
  ...
  [duoIndex,googleIndex,householdIndex,bridgeIndex,controllerIndex,contextIndex].forEach((idx)=>assert.ok(idx>=0,'canonical auth/household runtime script must be served'));
  assert.ok(duoIndex<googleIndex,'Firebase/legacy compatibility bootstrap must load before Google adapter');
