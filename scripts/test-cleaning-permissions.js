...
async function main(){
  ...
  const repo={
    ...
    updateRoom:async()=>{calls.push('updateRoom');return true;},
    removeRoom:async()=>{calls.push('removeRoom');return true;},
    createRoutineItem:async()=>{calls.push('createRoutineItem');return true;},
    removeRoutineItem:async()=>{calls.push('removeRoutineItem');return true;},
  ...
  const noop=()=>{};
  const document={documentElement:{},addEventListener:noop,querySelectorAll:()=>[],getElementById:()=>null};
  function MutationObserver(){this.observe=noop;}
  const sandbox={
    console,Promise,setTimeout:()=>1,clearTimeout:noop,setInterval:()=>1,clearInterval:noop,
    requestAnimationFrame:(fn)=>{fn();return 1;},MutationObserver,document,
    ...
    CleaningHouseholdRepository:repo,CleaningRoutineExperience:routineExperience,
    CleaningAvailabilityExperience:availability,CleaningSupplyExperience:supplies,CleaningPauseExperience:pause,
    addEventListener:noop,dispatchEvent:noop,showToast:noop
  };
  ...
  assert(caps.STRUCTURE&&caps.DESTRUCTIVE_STRUCTURE&&caps.PLANNING&&caps.ASSIGNMENTS&&caps.HOUSEHOLD_AVAILABILITY,'manager has normal and destructive structural controls');
  caps=permissions.capabilities(permissions.ROLE.MEMBER);
  assert(caps.STRUCTURE&&!caps.DESTRUCTIVE_STRUCTURE&&caps.PLANNING&&caps.ASSIGNMENTS&&caps.AVAILABILITY&&caps.SUPPLIES,'adult/member can manage rooms/routines but not destructive admin actions');
  caps=permissions.capabilities(permissions.ROLE.LIMITED);
  assert(!caps.STRUCTURE&&!caps.DESTRUCTIVE_STRUCTURE&&!caps.PLANNING&&!caps.ASSIGNMENTS&&!caps.SUPPLIES&&caps.EXECUTION&&caps.RESPOND&&caps.HELP,'limited profile only keeps execution/respond/help plus personal preferences');
  assert(source.includes("[data-cleaning-supply-form]',function(node){markHidden(node,!can(CAP.SUPPLIES))"),'supply creation UI follows SUPPLIES capability');
  assert(source.includes("[data-cleaning-room-add],#screen-cleaning [data-cleaning-room-edit]"),'room create/edit UI remains covered by structural capability');
  assert(source.includes("markHidden(node,!can(CAP.DESTRUCTIVE_STRUCTURE))"),'destructive room/routine actions remain separately guarded');
  if(!failed)ok('household roles map to generic adult-member Cleaning management with manager-only destructive actions');

  currentRole='owner';
  await repo.createRoom({name:'Badkamer'});
  await repo.removeRoom('room-old');
  assert(calls.includes('createRoom')&&calls.includes('removeRoom'),'manager can create and remove structure');

  currentRole='adult';
  await repo.createRoom({name:'Keuken'});
  await repo.updateRoom('room1',{name:'Keuken beneden'});
  await repo.createRoutineItem({roomId:'room1',title:'Aanrecht'});
  await repo.updateRoutineItem('r1',{title:'Stofzuigen grondig',intervalDays:7,estimatedMinutes:20,priority:'NORMAL',roomId:'room1',supplyIds:['s1']});
  assert(calls.filter((entry)=>entry==='createRoom').length>=2,'adult/member can create rooms in any household');
  assert(calls.includes('updateRoom')&&calls.includes('createRoutineItem')&&calls.includes('updateRoutineItem:r1'),'adult/member can edit rooms and manage routines');
  await rejects(repo.removeRoom('room1'),'adult/member must not permanently remove rooms');
  await rejects(repo.removeRoutineItem('r1'),'adult/member must not permanently remove routines');
  ...
  if(!failed)ok('adult/member has normal Cleaning management without destructive or household-wide admin powers');

  currentRole='child';
  await rejects(repo.createRoom({name:'Kinderbadkamer'}),'limited profile must not create rooms');
  await rejects(repo.createRoutineItem({roomId:'room1',title:'Dweilen'}),'limited profile must not create routines');
  ...
  if(!failed)ok('server-side Firebase rule hardening remains separated from this client functional milestone');
