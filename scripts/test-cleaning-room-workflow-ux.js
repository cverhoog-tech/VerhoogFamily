...
assert.ok(ux);
assert.strictEqual(ux.version,'0.1.1');
...
assert.ok(source.includes('display:none!important'),'three separate routine controls are visually collapsed behind one menu');
assert.ok(!source.includes('cleaning-approval-copy'));
assert.ok(!source.includes('cleaning-plan-actions > span'));

console.log('cleaning optional room name + compact routine actions: ok');
