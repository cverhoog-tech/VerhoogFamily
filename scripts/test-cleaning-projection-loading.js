...
assert.ok(templates.includes("import './cleaningQuickChoiceFeedback.js?v=1';"));
...
assert.ok(templates.indexOf('cleaningRoutineExperience.js?v=3') < templates.indexOf('cleaningQuickChoiceFeedback.js?v=1'));
assert.ok(templates.indexOf('cleaningQuickChoiceFeedback.js?v=1') < templates.indexOf('cleaningActivePlanReconciler.js?v=2'));
...
assert.ok(quickChoice.includes("var VERSION='0.2.0'"));
...
assert.ok(quickChoice.includes("window.scrollTo({top:pending.scrollTop,left:0,behavior:'auto'})"));
...
// Quick-choice feedback may observe the room list to preserve its position,
// but it must never touch the Planning approval nodes.
...
console.log('cleaning quick-choice feedback + rolling runtime order + single-owner guard: ok');
