...
**STEP 11 — Party quests is in progress. STEP 11.1 through STEP 11.6 are implementation/contract complete. STEP 11.2 invite/accept passed on real device. STEP 11.4 targeted-help Test 1 passed on a real device, while recipient/broadcast follow-up and STEP 11.3 leave remain pending. The Party Quest UX patch is real-device accepted for multi-start/Arcana icons, canonical new-task handoff and explicit `Later beslissen`. STEP 11.5 real-device Tests 1 and 2 passed; its final reload/no-duplicate XP Test 3 is still pending. STEP 11.6 is now fully real-device accepted: ordinary involved-Task notification, combined Party Quest completion + XP notification and reload/replay no-duplicate behavior all passed. STEP 11.7 has not started and still requires explicit product-owner approval.**
...
- [ ] STEP 11.3 real-device leave smoke still pending; may be bundled into a later STEP 11 acceptance sweep under accelerated validation mode unless it becomes release-blocking.
...
- [ ] STEP 11.4 remaining device help smoke: recipient accept/decline and household broadcast; may be bundled into a later STEP 11 acceptance sweep unless a blocker appears.
...
- [ ] STEP 11.5 real-device Test 3 pending: reload/reopen as that same participant and confirm the same Party Quest XP/reward is not granted a second time. This remains a reward/idempotency safety check and should still be verified before final STEP 11 acceptance, but it can be bundled with the later acceptance sweep.
...
- [x] STEP 11.6 real-device Test 3 PASS: after reload/reopen as account B, the same combined notification still existed only once; reconnect/replay created no duplicate.
- [x] STEP 11.6 REAL-DEVICE ACCEPTED on 2026-08-29.
...
- [ ] Real-device participant leave smoke pending; bundle into later STEP 11 acceptance sweep unless a blocker appears.
...
- [ ] Recipient accept/decline and household broadcast follow-up pending; bundle into later STEP 11 acceptance sweep unless a blocker appears.
...
- [ ] Real-device Test 3 pending: no duplicate XP/reward after reload; retain as a safety gate but verify in the bundled STEP 11 acceptance sweep.
...
- [x] Real-device Test 3 PASS: reload/reopen produced no duplicate combined notification.
- [x] Product owner accepted STEP 11.6 device behavior on 2026-08-29.
...
- [ ] STEP 11.9 — bundled real iPhone acceptance sweep under accelerated validation mode; separate only high-risk/blocking checks.
...
- Use accelerated validation mode: bundle non-critical device checks per checkpoint; keep high-risk/destructive/security/idempotency/release blockers explicit.
